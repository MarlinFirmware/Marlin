...
#define HOMING_BUMP_MM      { 5, 5, 2, 2, 2 }       // (mm for linear axes, ° for rotational axes) Backoff from endstops after first bump
...
//#define QUICK_HOME_ALL_NON_Z_AXES           // If G28 contains XY[, A[, B[, C[, U[, V[, W]]]]]] raise Z first, then move all axes simultaneously towards home, then home XYZIJKUVW individually.
...
// @section lcd
  ...
  #define MANUAL_FEEDRATE { 50*60, 50*60, 4*60, 2*60, 2*60, 2*60, 2*60, 2*60 } // (mm/min for linear axes, °/min for rotational axes) Feedrates for manual moves along X, Y, Z, E from panel
...
#define SAFE_BED_LEVELING_START_Z 160.0
