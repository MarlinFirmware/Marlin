...
{
  /* The Prusa bootloader hands over with the PLL already selected as the
   * system clock and with interrupts masked. The HAL refuses to change a
   * PLL that is in use, so reset the clock tree first, and unmask
   * interrupts so SysTick runs for HAL timeouts and delay(). */
  __enable_irq();
  HAL_RCC_DeInit();
