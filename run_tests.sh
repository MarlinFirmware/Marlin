...
function run
{
    ...
    cp $CFG $CFG.orig
    cat $CFG.orig | ./set-defines.sh MOTHERBOARD ENABLE_AUTO_BED_LEVELING AUTO_BED_LEVELING_ANY_POINTS > $CFG
    echo "Current defines:"
    grep -e MOTHERBOARD -e ENABLE_AUTO_BED_LEVELING -e AUTO_BED_LEVELING_ANY_POINTS -e DELTA $CFG
    sleep 2
