...
function run
{
    ...
    cp $CFG $CFG.orig
    cat $CFG.orig | ./set-defines.sh MOTHERBOARD ENABLE_AUTO_BED_LEVELING AUTO_BED_LEVELING_ANY_POINTS Z_SAFE_HOMING_FIXED_POINT CUSTOM_M_CODES > $CFG
    echo "Current defines:"
    grep -e MOTHERBOARD -e ENABLE_AUTO_BED_LEVELING -e AUTO_BED_LEVELING_ANY_POINTS -e Z_SAFE_HOMING_FIXED_POINT -e DELTA -e CUSTOM_M_CODES $CFG
    sleep 2
...
MOTHERBOARD=81 ENABLE_AUTO_BED_LEVELING=1 AUTO_BED_LEVELING_ANY_POINTS=1 Z_SAFE_HOMING_FIXED_POINT=1 CUSTOM_M_CODES=1
run
