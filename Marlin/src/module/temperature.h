...
typedef struct HeaterInfo : public TempInfo {
  ...
  bool is_below_target(const celsius_t offs=0) const { return (target - celsius >= offs); }
  bool is_above_target(const celsius_t offs=0) const { return (celsius - target >= offs); }
} heater_info_t;
