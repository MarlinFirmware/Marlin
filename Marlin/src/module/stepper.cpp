...
#define TRIPLE_SEPARATE_APPLY_STEP(A,V)           \
  ...
  #define X_INIT_STEP() do{ X_STEP_INIT(); X_STEP_PULSE_STOP(INVERT_X_STEP_PIN); disable_X(); }while(0)
  #define X_APPLY_STEP_PULSE_START() X_STEP_PULSE_START(INVERT_X_STEP_PIN)
  #define X_APPLY_STEP_PULSE_STOP() X_STEP_PULSE_STOP(INVERT_X_STEP_PIN)
  ...
  #define Y_INIT_STEP() do{ Y_STEP_INIT(); Y_STEP_PULSE_STOP(INVERT_Y_STEP_PIN); disable_Y(); }while(0)
  #define Y_APPLY_STEP_PULSE_START() Y_STEP_PULSE_START(INVERT_Y_STEP_PIN)
  #define Y_APPLY_STEP_PULSE_STOP() Y_STEP_PULSE_STOP(INVERT_Y_STEP_PIN)
  ...
  #define Z_INIT_STEP() do{ Z_STEP_INIT(); Z_STEP_PULSE_STOP(INVERT_Z_STEP_PIN); disable_Z(); }while(0)
  #define Z_APPLY_STEP_PULSE_START() Z_STEP_PULSE_START(INVERT_Z_STEP_PIN)
  #define Z_APPLY_STEP_PULSE_STOP() Z_STEP_PULSE_STOP(INVERT_Z_STEP_PIN)
...
#define _AXIS_DISABLE(INDEX) disable_E## INDEX
#define E_INIT_ENABLE(INDEX) do{ E_ENABLE_INIT(E## INDEX); if (!E_ENABLE_ON) E_ENABLE_WRITE(E## INDEX, HIGH); }while(0)
#define E_INIT_DIR(INDEX) E_DIR_INIT(E## INDEX)
#define E_INIT_STEP(INDEX) do{ E_STEP_INIT(E## INDEX); E_STEP_PULSE_STOP(E## INDEX); _AXIS_DISABLE(INDEX)(); }while(0)

#if ENABLED(MIXING_EXTRUDER)
  #define E_APPLY_STEP_PULSE_START() E_STEP_PULSE_START(mixer.get_next_stepper())
  #define E_APPLY_STEP_PULSE_STOP() E_STEP_PULSE_STOP(mixer.get_stepper())

  #define E_APPLY_STEP_FIRST_PHASE_WIDTH MINIMUM_STEPPER_PULSE / 2
  #define E_APPLY_STEP_SECOND_PHASE_NEED true
  #if E_APPLY_STEP_SECOND_PHASE_WIDTH
    #define E_APPLY_STEP_SECOND_PHASE_WIDTH MINIMUM_STEPPER_PULSE / 2
  #else
    #define E_APPLY_STEP_SECOND_PHASE_WIDTH 0
  #endif
#else
  #define E_APPLY_STEP_PULSE_START() E_STEP_PULSE_START(stepper_extruder)
  #define E_APPLY_STEP_PULSE_STOP() E_STEP_PULSE_STOP(stepper_extruder)
...
static constexpr hal_timer_t step_pulse_first_phase_ticks[] = { // Precalculated step delays for each XYZE movement combination
  GET_PULSE_TICKS(0)
  , GET_PULSE_TICKS(X_APPLY_STEP_FIRST_PHASE_WIDTH)
  , GET_PULSE_TICKS(Y_APPLY_STEP_FIRST_PHASE_WIDTH)
  , GET_PULSE_TICKS(MAX(X_APPLY_STEP_FIRST_PHASE_WIDTH, Y_APPLY_STEP_FIRST_PHASE_WIDTH))
  , GET_PULSE_TICKS(Z_APPLY_STEP_FIRST_PHASE_WIDTH)
  , GET_PULSE_TICKS(MAX(X_APPLY_STEP_FIRST_PHASE_WIDTH, Z_APPLY_STEP_FIRST_PHASE_WIDTH))
  , GET_PULSE_TICKS(MAX(Y_APPLY_STEP_FIRST_PHASE_WIDTH, Z_APPLY_STEP_FIRST_PHASE_WIDTH))
  , GET_PULSE_TICKS(MAX(X_APPLY_STEP_FIRST_PHASE_WIDTH, Y_APPLY_STEP_FIRST_PHASE_WIDTH, Z_APPLY_STEP_FIRST_PHASE_WIDTH))
  #if DISABLED(LIN_ADVANCE) && DISABLED(MIXING_EXTRUDER)
    , GET_PULSE_TICKS(E_APPLY_STEP_FIRST_PHASE_WIDTH)
    , GET_PULSE_TICKS(MAX(X_APPLY_STEP_FIRST_PHASE_WIDTH, E_APPLY_STEP_FIRST_PHASE_WIDTH))
    , GET_PULSE_TICKS(MAX(Y_APPLY_STEP_FIRST_PHASE_WIDTH, E_APPLY_STEP_FIRST_PHASE_WIDTH))
    , GET_PULSE_TICKS(MAX(X_APPLY_STEP_FIRST_PHASE_WIDTH, Y_APPLY_STEP_FIRST_PHASE_WIDTH, E_APPLY_STEP_FIRST_PHASE_WIDTH))
    , GET_PULSE_TICKS(MAX(Z_APPLY_STEP_FIRST_PHASE_WIDTH, E_APPLY_STEP_FIRST_PHASE_WIDTH))
    , GET_PULSE_TICKS(MAX(X_APPLY_STEP_FIRST_PHASE_WIDTH, Z_APPLY_STEP_FIRST_PHASE_WIDTH, E_APPLY_STEP_FIRST_PHASE_WIDTH))
    , GET_PULSE_TICKS(MAX(Y_APPLY_STEP_FIRST_PHASE_WIDTH, Z_APPLY_STEP_FIRST_PHASE_WIDTH, E_APPLY_STEP_FIRST_PHASE_WIDTH))
    , GET_PULSE_TICKS(MAX(X_APPLY_STEP_FIRST_PHASE_WIDTH, Y_APPLY_STEP_FIRST_PHASE_WIDTH, Z_APPLY_STEP_FIRST_PHASE_WIDTH, E_APPLY_STEP_FIRST_PHASE_WIDTH))
  #endif
};

static constexpr hal_timer_t step_pulse_second_phase_ticks[] = { // Precalculated step delays for each XYZE movement combination
  GET_PULSE_TICKS(0)
  , GET_PULSE_TICKS(X_APPLY_STEP_SECOND_PHASE_WIDTH)
  , GET_PULSE_TICKS(Y_APPLY_STEP_SECOND_PHASE_WIDTH)
  , GET_PULSE_TICKS(MAX(X_APPLY_STEP_SECOND_PHASE_WIDTH, Y_APPLY_STEP_SECOND_PHASE_WIDTH))
  , GET_PULSE_TICKS(Z_APPLY_STEP_SECOND_PHASE_WIDTH)
  , GET_PULSE_TICKS(MAX(X_APPLY_STEP_SECOND_PHASE_WIDTH, Z_APPLY_STEP_SECOND_PHASE_WIDTH))
  , GET_PULSE_TICKS(MAX(Y_APPLY_STEP_SECOND_PHASE_WIDTH, Z_APPLY_STEP_SECOND_PHASE_WIDTH))
  , GET_PULSE_TICKS(MAX(X_APPLY_STEP_SECOND_PHASE_WIDTH, Y_APPLY_STEP_SECOND_PHASE_WIDTH, Z_APPLY_STEP_SECOND_PHASE_WIDTH))
  #if DISABLED(LIN_ADVANCE) && DISABLED(MIXING_EXTRUDER)
    , GET_PULSE_TICKS(E_APPLY_STEP_SECOND_PHASE_WIDTH)
    , GET_PULSE_TICKS(MAX(X_APPLY_STEP_SECOND_PHASE_WIDTH, E_APPLY_STEP_SECOND_PHASE_WIDTH))
    , GET_PULSE_TICKS(MAX(Y_APPLY_STEP_SECOND_PHASE_WIDTH, E_APPLY_STEP_SECOND_PHASE_WIDTH))
    , GET_PULSE_TICKS(MAX(X_APPLY_STEP_SECOND_PHASE_WIDTH, Y_APPLY_STEP_SECOND_PHASE_WIDTH, E_APPLY_STEP_SECOND_PHASE_WIDTH))
    , GET_PULSE_TICKS(MAX(Z_APPLY_STEP_SECOND_PHASE_WIDTH, E_APPLY_STEP_SECOND_PHASE_WIDTH))
    , GET_PULSE_TICKS(MAX(X_APPLY_STEP_SECOND_PHASE_WIDTH, Z_APPLY_STEP_SECOND_PHASE_WIDTH, E_APPLY_STEP_SECOND_PHASE_WIDTH))
    , GET_PULSE_TICKS(MAX(Y_APPLY_STEP_SECOND_PHASE_WIDTH, Z_APPLY_STEP_SECOND_PHASE_WIDTH, E_APPLY_STEP_SECOND_PHASE_WIDTH))
    , GET_PULSE_TICKS(MAX(X_APPLY_STEP_SECOND_PHASE_WIDTH, Y_APPLY_STEP_SECOND_PHASE_WIDTH, Z_APPLY_STEP_SECOND_PHASE_WIDTH, E_APPLY_STEP_SECOND_PHASE_WIDTH))
  #endif
};

static constexpr uint8_t step_pulse_second_phase_need =
  (X_APPLY_STEP_SECOND_PHASE_NEED << X_AXIS)
  | (Y_APPLY_STEP_SECOND_PHASE_NEED << Y_AXIS)
  | (Z_APPLY_STEP_SECOND_PHASE_NEED << Z_AXIS)
  #if DISABLED(LIN_ADVANCE) && DISABLED(MIXING_EXTRUDER)
    | (E_APPLY_STEP_SECOND_PHASE_NEED << E_AXIS)
  #endif
  ;
...
void Stepper::stepper_pulse_phase_isr() {
  ...
  do {
    uint8_t axis_has_step = 0;

    #define _APPLY_STEP_PULSE_START(AXIS) AXIS ##_APPLY_STEP_PULSE_START
    #define _APPLY_STEP_PULSE_STOP(AXIS) AXIS ##_APPLY_STEP_PULSE_STOP
    ...
    #define PULSE_START(AXIS) do{ \
      ...
      if (delta_error[_AXIS(AXIS)] >= 0) { \
        _APPLY_STEP_PULSE_START(AXIS)(); \
        axis_has_step |= 1 << _AXIS(AXIS); \
    ...
    if (step_pulse_first_phase_ticks[axis_has_step] > GET_PULSE_TICKS(0)) {
      pulse_end += step_pulse_first_phase_ticks[axis_has_step];
    ...
    if (events_to_do && (step_pulse_second_phase_need & axis_has_step)) {
      pulse_end += step_pulse_second_phase_ticks[axis_has_step];
      // Just wait for the requested pulse duration
      while (HAL_timer_get_count(PULSE_TIMER_NUM) < pulse_end) { /* nada */ }
    }
