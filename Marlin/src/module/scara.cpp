...
float segments_per_second = TERN(AXEL_TPARA, TPARA_SEGMENTS_PER_SECOND, SCARA_SEGMENTS_PER_SECOND);
  ...
  void forward_kinematics(const_float_t a, const_float_t b) {
    const float a_sin = sin(RADIANS(a)) * L1,
                ...
                b_sin = sin(RADIANS(b PLUS_TERN0(MP_SCARA, a))) * L2,
                b_cos = cos(RADIANS(b PLUS_TERN0(MP_SCARA, a))) * L2;
  ...
  void inverse_kinematics(const xyz_pos_t &raw) {
    ...
    delta.set(DEGREES(THETA), DEGREES(PSI PLUS_TERN0(MORGAN_SCARA, THETA)), raw.z);
