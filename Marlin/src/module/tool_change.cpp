...
#include "../core/debug_out.h"
  ...
  inline void dualx_tool_change(const uint8_t new_tool, bool &no_move) {
    ...
    if (dual_x_carriage_mode == DXC_AUTO_PARK_MODE                  // If Auto-Park mode is enabled
      ...
      line_to_current_position(planner.settings.max_feedrate_mm_s[X_AXIS] OPTARG(HAS_ROTATIONAL_AXES, 0.0f));   // Park the current head
      planner.synchronize();
    }
 ...
 */
  ...
  void tool_change_prime() {
    ...
    if (!too_cold(active_extruder)) {
      ...
      #if ENABLED(TOOLCHANGE_PARK)
        if (ok) {
          TERN_(TOOLCHANGE_NO_RETURN, destination.set(current_position.x, current_position.y));
          TERN_(HAS_ROTATIONAL_AXES, hints.fr_deg_s = DPM_TO_DPS(TOOLCHANGE_PARK_XY_FEEDRATE));
          const float destz = destination.z;
          destination.z = current_position.z;
          planner.buffer_line(destination, MMM_TO_MMS(TOOLCHANGE_PARK_XY_FEEDRATE), active_extruder OPTARG(HAS_ROTATIONAL_AXES, hints));
          current_position = destination;
          current_position.z = destz;
          planner.buffer_line(destination, planner.settings.max_feedrate_mm_s[Z_AXIS], active_extruder OPTARG(HAS_ROTATIONAL_AXES, hints));
        }
      #endif
