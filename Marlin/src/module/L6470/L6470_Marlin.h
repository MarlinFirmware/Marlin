...
#define MAX_L6470  13    // maximum number of axis in a Marlin system
...
#define HAS_L6470_EXTRUDER (    AXIS_DRIVER_TYPE(E0, ST_L6470) || AXIS_DRIVER_TYPE(E1, ST_L6470) || AXIS_DRIVER_TYPE(E2, ST_L6470) \
  ...
  extern uint8_t L6470_E_dir_commands[6];  // array to hold direction command for each extruder
#endif
...
bool L6470_get_user_input( uint8_t* p_driver_count, uint8_t axis_index[3], char axis_mon[3][3],
                           ...
                           bool over_current_flag, uint8_t* p_OCD_TH_val, uint8_t* p_STALL_TH_val, uint16_t* p_over_current_threshold );

void print_bin(uint16_t val);

void L6470_error_status_decode(uint16_t status, uint8_t axis);
