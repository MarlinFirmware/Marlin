...
uint32_t Stepping::advance_until_step() {
  ...
  #define RUN_AXIS(A) if NOMORE(iterations, uint32_t((uint64_t(space_q32.A) * advance_dividend_reciprocal.A) >> 32));
...
uint32_t Stepping::plan() {
  ...
  #define _DIVIDEND_RECIP(A) advance_dividend_reciprocal.A = (stepper_plan.advance_dividend_q0_32.A == 0) ? UINT32_MAX : UINT32_MAX / stepper_plan.advance_dividend_q0_32.A;
