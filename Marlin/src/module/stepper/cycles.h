...
#include "../../core/types.h"
   ...
   */
  constexpr cycles_t timer_read_add_and_store_cycles = 34UL;

  // The base ISR
  constexpr cycles_t isr_base_cycles = 770UL;

  // Linear advance base time is 64 cycles
  constexpr cycles_t isr_la_base_cycles = TERN0(LIN_ADVANCE, 64UL);

  // S curve interpolation adds 40 cycles
  constexpr cycles_t isr_s_curve_cycles = TERN0(S_CURVE_ACCELERATION, TERN(STM32G0B1xx, 500UL, 40UL));

  // Input shaping base time
  constexpr cycles_t isr_shaping_base_cycles = TERN0(HAS_ZV_SHAPING, 180UL);

  // Stepper Loop base cycles
  constexpr cycles_t isr_loop_base_cycles = 4UL;

  // And each stepper (start + stop pulse) takes in worst case
  constexpr cycles_t isr_stepper_cycles = 100UL;

#else

  // Cycles to perform actions in START_TIMED_PULSE
  constexpr cycles_t timer_read_add_and_store_cycles = 13UL;

  // The base ISR
  constexpr cycles_t isr_base_cycles = 882UL;

  // Linear advance base time is 32 cycles
  constexpr cycles_t isr_la_base_cycles = TERN0(LIN_ADVANCE, 30UL);

  // S curve interpolation adds 160 cycles
  constexpr cycles_t isr_s_curve_cycles = TERN0(S_CURVE_ACCELERATION, 160UL);

  // Input shaping base time
  constexpr cycles_t isr_shaping_base_cycles = TERN0(HAS_ZV_SHAPING, 290UL);

  // Stepper Loop base cycles
  constexpr cycles_t isr_loop_base_cycles = 32UL;

  // And each stepper (start + stop pulse) takes in worst case
  constexpr cycles_t isr_stepper_cycles = 60UL;

#endif

// If linear advance is disabled, the loop also handles them
constexpr cycles_t isr_mixing_stepper_cycles = (0UL
...
constexpr cycles_t min_isr_loop_cycles = isr_mixing_stepper_cycles + LOGICAL_AXES * isr_stepper_cycles;

// Calculate the minimum pulse times (high and low)
#if defined(MINIMUM_STEPPER_PULSE_NS) && defined(MAXIMUM_STEPPER_RATE)
  constexpr nanos_t _min_step_period_ns = 1000000000UL / (MAXIMUM_STEPPER_RATE),
  ...
  constexpr nanos_t _min_pulse_high_ns = MINIMUM_STEPPER_PULSE_NS,
                     _min_pulse_low_ns = _min_pulse_high_ns;
#elif defined(MAXIMUM_STEPPER_RATE)
  // Assume equal high and low pulse durations
  constexpr nanos_t _min_pulse_high_ns = 500000000UL / MAXIMUM_STEPPER_RATE,
                     _min_pulse_low_ns = _min_pulse_high_ns;
#else
...
constexpr cycles_t min_stepper_pulse_cycles = _min_pulse_high_ns * CYCLES_PER_MICROSECOND / 1000;
...
constexpr cycles_t isr_loop_cycles(const int R) { return ((isr_loop_base_cycles + min_isr_loop_cycles + min_stepper_pulse_cycles) * ((1UL << R) - 1) + _MAX(min_isr_loop_cycles, min_stepper_pulse_cycles)); }

// Model input shaping as an extra loop call
constexpr cycles_t isr_shaping_loop_cycles(const int R) {
  return (
    #if HAS_ZV_SHAPING
...
// If linear advance is enabled, then it is handled separately
  ...
  constexpr cycles_t min_isr_la_loop_cycles = (isr_stepper_cycles
    // ToDo: ???
...
constexpr cycles_t isr_la_loop_cycles = TERN0(LIN_ADVANCE, _MAX(min_stepper_pulse_cycles, min_isr_la_loop_cycles));

// Estimate the total ISR execution time in cycles given a step-per-ISR shift multiplier
constexpr cycles_t isr_execution_cycles(const int R) { return (isr_base_cycles + isr_s_curve_cycles + isr_shaping_base_cycles + isr_loop_cycles(R) + isr_shaping_loop_cycles(R) + isr_la_base_cycles + isr_la_loop_cycles) >> R; }

// The maximum allowable stepping frequency when doing 1x stepping (in Hz)
constexpr hertz_t max_step_isr_frequency_1x = (F_CPU) / isr_execution_cycles(0);
constexpr hertz_t max_step_isr_frequency_sh(const int S) { return ((F_CPU) / isr_execution_cycles(S)) >> S; }
...
constexpr hertz_t min_step_isr_frequency = max_step_isr_frequency_1x >> 1;
