...
void home_delta() {
  ...
  // Disable stealthChop if used. Enable diag1 pin on driver.
    ...
    IF_ENABLED(I_SENSORLESS, sensorless_t stealth_states_i = start_sensorless_homing_per_axis(I_AXIS));
    IF_ENABLED(J_SENSORLESS, sensorless_t stealth_states_j = start_sensorless_homing_per_axis(J_AXIS));
    IF_ENABLED(K_SENSORLESS, sensorless_t stealth_states_k = start_sensorless_homing_per_axis(K_AXIS));
    IF_ENABLED(U_SENSORLESS, sensorless_t stealth_states_u = start_sensorless_homing_per_axis(U_AXIS));
    IF_ENABLED(V_SENSORLESS, sensorless_t stealth_states_v = start_sensorless_homing_per_axis(V_AXIS));
    IF_ENABLED(W_SENSORLESS, sensorless_t stealth_states_w = start_sensorless_homing_per_axis(W_AXIS));
  ...
  planner.synchronize();
  IF_ENABLED(HAS_DELTA_SENSORLESS_PROBING, endstops.report_states());
  ...
  // Re-enable stealthChop if used. Disable diag1 pin on driver.
    ...
    IF_ENABLED(I_SENSORLESS, end_sensorless_homing_per_axis(I_AXIS, stealth_states_i));
    IF_ENABLED(J_SENSORLESS, end_sensorless_homing_per_axis(J_AXIS, stealth_states_j));
    IF_ENABLED(K_SENSORLESS, end_sensorless_homing_per_axis(K_AXIS, stealth_states_k));
    IF_ENABLED(U_SENSORLESS, end_sensorless_homing_per_axis(U_AXIS, stealth_states_u));
    IF_ENABLED(V_SENSORLESS, end_sensorless_homing_per_axis(V_AXIS, stealth_states_v));
    IF_ENABLED(W_SENSORLESS, end_sensorless_homing_per_axis(W_AXIS, stealth_states_w));
    #if SENSORLESS_STALLGUARD_DELAY
      safe_delay(SENSORLESS_STALLGUARD_DELAY); // Short delay needed to settle
    #endif
  #endif
