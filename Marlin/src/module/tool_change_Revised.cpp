 ...
 */
  ...
  void extruder_prime(const uint8_t toolID) {
    ...
    if ( toolchange_settings.extra_prime > 0 ) {
      unscaled_e_move(toolchange_settings.swap_length, MMM_TO_MMS(fr)); // Unretract (or Prime)
      unscaled_e_move(toolchange_settings.extra_prime, MMM_TO_MMS(toolchange_settings.prime_speed)); // Extra Prime Distance
    } else {
      // Unretract distance that has a negative 'extra_prime' modifier
  ...
  void tool_change_prime() {
    if (toolchange_settings.extra_prime > 0
      ...
      extruder_cutting_recover(); // Cutting recover
      ...
      if (should_move) {
        ...
        if (can_move_away) {
          ...
          if (should_swap && !too_cold) {
            extruder_cutting_recover(); // New extruder primed and set to 0
