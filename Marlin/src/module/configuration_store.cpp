 ...
 *  354  M666 XYZ  delta_endstop_adj                (float x3)
 ...
 * DELTA_AUTO_CALIBRATE:                            4 bytes
 *  396  G33 Z     additional_z_offset              (float)
 *
...
void MarlinSettings::postprocess() {
  ...
  bool MarlinSettings::save() {
    ...
    // 12 floats for DELTA / [XYZ]_DUAL_ENDSTOPS
      ...
      #endif
      EEPROM_WRITE(additional_z_offset);       // 1 float
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      //
        ...
        EEPROM_READ(delta_tower_angle_trim);    // 3 floats
          ...
          float additional_z_offset;
        #endif
        EEPROM_READ(additional_z_offset);       // 1 float
...
void MarlinSettings::reset() {
  ...
  // Applies to all MBL and ABL
    ...
    delta_height = DELTA_HEIGHT;
      ...
      additional_z_offset = ADDITIONAL_Z_OFFSET;
    #endif
