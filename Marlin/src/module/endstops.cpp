  ...
  void Endstops::set_homing_current(const bool onoff) {
    #define HAS_CURRENT_HOME(N) (defined(N##_CURRENT_HOME) && N##_CURRENT_HOME != N##_CURRENT)
      ...
      if (onoff) {
            ...
            debug_current_on(PSTR("X"), saved_current_X, stepperX.getMilliamps());
          #endif
          #if HAS_CURRENT_HOME(Y)
            saved_current_Y = stepperY.getMilliamps();
            stepperY.rms_current(saved_current_Y - REDUCE_CURRENT);
            debug_current_on(PSTR("Y"), saved_current_Y, stepperY.getMilliamps());
          #endif
        #endif
        #if HAS_CURRENT_HOME(Z)
          saved_current_Z = stepperZ.getMilliamps();
          stepperZ.rms_current(saved_current_Z - REDUCE_CURRENT);
          debug_current_on(PSTR("Z"), saved_current_Z, stepperZ.getMilliamps());
        #endif
