...
enum EndstopEnum : char {
  ...
  // Bed Probe state shared with Z_MIN is an alias
  #if !USE_Z_MIN_PROBE
    ES_ITEM(HAS_Z_PROBE_STATE, Z_MIN_PROBE = Z_MIN)
  #endif

  // Endstop aliases
  ES_ITEM(HAS_X_STATE, X_ENDSTOP = TERN(X_HOME_TO_MAX, X_MAX, X_MIN))
  ES_ITEM(HAS_X2_STATE, X2_ENDSTOP = TERN(X_HOME_TO_MAX, X2_MAX, X2_MIN))
  ES_ITEM(HAS_Y_STATE, Y_ENDSTOP = TERN(Y_HOME_TO_MAX, Y_MAX, Y_MIN))
  ES_ITEM(HAS_Y2_STATE, Y2_ENDSTOP = TERN(Y_HOME_TO_MAX, Y2_MAX, Y2_MIN))
  #if HOMING_Z_WITH_PROBE
    Z_ENDSTOP = Z_MIN_PROBE, // "Z" endstop alias when homing with the probe
  #elif HAS_Z_STATE
    Z_ENDSTOP = TERN(Z_HOME_TO_MAX, Z_MAX, Z_MIN),
  #endif
  ES_ITEM(HAS_Z2_STATE, Z2_ENDSTOP = TERN(Z_HOME_TO_MAX, Z2_MAX, Z2_MIN))
  ES_ITEM(HAS_Z3_STATE, Z3_ENDSTOP = TERN(Z_HOME_TO_MAX, Z3_MAX, Z3_MIN))
  ES_ITEM(HAS_Z4_STATE, Z4_ENDSTOP = TERN(Z_HOME_TO_MAX, Z4_MAX, Z4_MIN))
  ES_ITEM(HAS_I_STATE, I_ENDSTOP = TERN(I_HOME_TO_MAX, I_MAX, I_MIN))
  ES_ITEM(HAS_J_STATE, J_ENDSTOP = TERN(J_HOME_TO_MAX, J_MAX, J_MIN))
  ES_ITEM(HAS_K_STATE, K_ENDSTOP = TERN(K_HOME_TO_MAX, K_MAX, K_MIN))
  ES_ITEM(HAS_U_STATE, U_ENDSTOP = TERN(U_HOME_TO_MAX, U_MAX, U_MIN))
  ES_ITEM(HAS_V_STATE, V_ENDSTOP = TERN(V_HOME_TO_MAX, V_MAX, V_MIN))
  ES_ITEM(HAS_W_STATE, W_ENDSTOP = TERN(W_HOME_TO_MAX, W_MAX, W_MIN))
};
