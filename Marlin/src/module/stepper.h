...
//
  ...
  typedef struct {
    float A, B, C;
    void reset() { 
        A = #ifdef NONLINEAR_FACTOR_A ? NONLINEAR_FACTOR_A : 0.0f;
        B = #ifdef NONLINEAR_FACTOR_B ? NONLINEAR_FACTOR_B : 0.0f;
        C = #ifdef NONLINEAR_FACTOR_C ? NONLINEAR_FACTOR_C : 1.0f;
        }
  } nonlinear_coeff_t;
