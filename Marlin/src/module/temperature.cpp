...
float Temperature::get_pid_output(const int8_t e) {
  ...
  float pid_output;
      ...
      if (temp_hotend[HOTEND_INDEX].target == 0
        ...
        const float max_power_over_i_gain = (float)PID_MAX / PID_PARAM(Ki, HOTEND_INDEX)  - (float)MIN_POWER;
        ...
        pid_output = work_pid[HOTEND_INDEX].Kp + work_pid[HOTEND_INDEX].Ki + work_pid[HOTEND_INDEX].Kd + (float)MIN_POWER;
