...
#if HAS_HEATED_BED
  #ifdef LONGER3D
    //#define STR_CONNECT(S,M)  S##M  
    #ifndef Chinese
      #define _BED_PSTR(M,E) (E) == -1 ? PSTR(M) :
    #else
      #define _BED_PSTR(M,E) (E) == -1 ? "ÈÈ´²¼ÓÈÈÊ§°Ü":
    #endif
  #else
      #define _BED_PSTR(M,E) (E) == -1 ? PSTR(M) :
  #endif
...
void Temperature::_temp_error(const int8_t heater, PGM_P const serial_msg, PGM_P const lcd_msg) {
  ...
  if(!is_printing)
  {
    //Serial1.println(lcd_msg);
    Display_Screen.LGT_Tempabnormal_Warning(lcd_msg);
    return;
  }
  #endif
...
void Temperature::updateTemperaturesFromRawValues() {
  ...
  ///////////////////////////////
 // Serial1.print("E=");Serial1.println((int16_t)temp_hotend[0].current);
  //////////////////////////////////
  #if HAS_HEATED_BED
    temp_bed.current = analog_to_celsius_bed(temp_bed.raw);
     ///////////////////////////////
  //Serial1.print("B=");Serial1.println((int16_t)temp_bed.current);
  //////////////////////////////////
  #endif
  #if HAS_TEMP_CHAMBER
