...
void prepare_line_to_destination() {
  ...
  bool homing_needed_error(uint8_t axis_bits/*=0x07*/) {
    if ((axis_bits = axes_should_home(axis_bits))) {
      ...
      char *p = sprint_str_P(msg, home_first);
      if (TEST(axis_bits, X_AXIS)) p = sprint_char(p, 'X');
      if (TEST(axis_bits, Y_AXIS)) p = sprint_char(p, 'Y');
      if (TEST(axis_bits, Z_AXIS)) p = sprint_char(p, 'Z');
      SERIAL_ECHO_START();
      SERIAL_ECHOLN(msg);
