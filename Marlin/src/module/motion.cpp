  ...
  inline bool line_to_destination_kinematic() {
    ...
    while (--segments) {
      ...
      if (!planner.buffer_line(raw, scaled_fr_mm_s, active_extruder
        , PlannerHints(cartesian_segment_mm OPTARG(SCARA_FEEDRATE_SCALING, inv_duration)))) break;
    }

    // Ensure last segment arrives at target location.
    planner.buffer_line(destination, scaled_fr_mm_s, active_extruder
      , PlannerHints(cartesian_segment_mm OPTARG(SCARA_FEEDRATE_SCALING, inv_duration)));
