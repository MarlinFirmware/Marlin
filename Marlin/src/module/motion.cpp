    ...
    float Motion::homeaxis_bl(const AxisEnum axis, const feedRate_t fr_mm_fast, const feedRate_t fr_mm_slow) {
      ...
      if (ENABLED(USER_LEVEL_DEBUG))
        SERIAL_ECHOLNPGM("\nhomeaxis_bl start to move z * 100 = ", axis * 100,
                          "\nhomeaxis_bl current scalin * 100 = ", (int)(motion.max_axis_length(TERN(DELTA, Z_AXIS, axis)) * axis_home_dir * 100),
                          );
      ...
      do_homing_move(axis, 0.90f * motion.max_axis_length(TERN(DELTA, Z_AXIS, axis)) * axis_home_dir, MMM_TO_MMS(fr_mm_fast));
      // do_homing_move(axis, 1.5f * motion.max_axis_length(TERN(DELTA, Z_AXIS, axis)) * axis_home_dir);//Z轴向下移动，移动距离为当前 Z_MAX_POS x 1.5f,当bltouch有反馈信号时，停止运行
      ...
      //do_homing_move(axis, 0.75f * motion.max_axis_length(TERN(DELTA, Z_AXIS, axis)) * axis_home_dir, MMM_TO_MMS(Z_PROBE_FEEDRATE_FAST * 15));
      do_homing_move(axis, 0.25f * motion.max_axis_length(TERN(DELTA, Z_AXIS, axis)) * axis_home_dir,  MMM_TO_MMS(fr_mm_slow));//Z轴向下移动，移动距离为当前 Z_MAX_POS x 1.5f,当bltouch有反馈信号时，停止运行
