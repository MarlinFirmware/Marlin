  ...
  public:
    ...
    static void check_axes_activity();
      ...
      FORCE_INLINE static void skew(float &cx, float &cy, const float &cz) {
        if (COORDINATE_WITHIN(cx, X_MIN_POS + 1, X_MAX_POS) && COORDINATE_WITHIN(cy, Y_MIN_POS + 1, Y_MAX_POS)) {
          const float sx = cx - cy * skew_factor.xy - cz * (skew_factor.xz - (skew_factor.xy * skew_factor.yz)),
                      sy = cy - cz * skew_factor.yz;
          if (COORDINATE_WITHIN(sx, X_MIN_POS, X_MAX_POS) && COORDINATE_WITHIN(sy, Y_MIN_POS, Y_MAX_POS)) {
            cx = sx; cy = sy;
          }
        }
      }
      ...
      FORCE_INLINE static void unskew(float &cx, float &cy, const float &cz) {
        if (COORDINATE_WITHIN(cx, X_MIN_POS, X_MAX_POS) && COORDINATE_WITHIN(cy, Y_MIN_POS, Y_MAX_POS)) {
