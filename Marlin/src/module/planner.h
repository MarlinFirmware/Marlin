...
typedef struct PlannerSettings {
  // (steps/mm) M92 XYZE - Steps per millimeter for each axis
    ...
    uint32_t max_acceleration_mm_per_s2[DISTINCT_AXES]; // (mm/s^2) M201 XYZE - Max accelerations
  feedRate_t max_feedrate_mm_s[DISTINCT_AXES];          // (mm/s)   M203 XYZE - Max speeds
       float acceleration,                              // (mm/s^2) M204 P - Normal acceleration. DEFAULT ACCELERATION for all printing moves
             retract_acceleration,                      // (mm/s^2) M204 R - Retract acceleration. Filament pull-back and push-forward while standing still in the other axes
             travel_acceleration;                       // (mm/s^2) M204 T - Travel acceleration. DEFAULT ACCELERATION for all NON printing moves
    uint32_t min_segment_time_us;                       // (µs)     M205 B - Minimum segment time
