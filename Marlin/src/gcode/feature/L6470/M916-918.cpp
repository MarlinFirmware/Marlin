  ...
  do {
      ...
      switch (test_phase) {
        ...
        case 2: {
          if (status_composite & (sh.STATUS_AXIS_STEP_LOSS_A | sh.STATUS_AXIS_STEP_LOSS_B)) {
            ...
            if (STALL_TH_val >= sh.AXIS_STALL_TH_MAX) {
              STALL_TH_val = sh.AXIS_STALL_TH_MAX;  // limit to max
    ...
    if (test_phase != 4) {
      for (j = 0; j < driver_count; j++) {                       // update threshold(s)
        ...
        L64xx_MARLIN.set_param(axis_index[j], L6470_STALL_TH, STALL_TH_val);
        if (L64xx_MARLIN.get_param(axis_index[j], L6470_OCD_TH) != OCD_TH_val) L6470_ECHOLNPGM("OCD mismatch");
        if (L64xx_MARLIN.get_param(axis_index[j], L6470_STALL_TH) != STALL_TH_val) L6470_ECHOLNPGM("STALL mismatch");
      }
    }
...
void GcodeSuite::M918() {
  ...
  for (j = 0; j < driver_count; j++)
    L64xx_MARLIN.set_param(axis_index[j], L6470_STEP_MODE, m_bits);   // set microsteps
