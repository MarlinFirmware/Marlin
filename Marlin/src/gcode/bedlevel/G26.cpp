...
typedef struct {
  ...
  void connect_neighbor_with_line(const xy_int8_t &p1, int8_t dx, int8_t dy) {
    ...
    if (p2.x < 0 || p2.x >= TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS_X, GRID_MAX_POINTS_X)) return;
    if (p2.y < 0 || p2.y >= TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS_Y, GRID_MAX_POINTS_Y)) return;
...
void GcodeSuite::G26() {
  ...
  #if HAS_MARLINUI_MENU
    g26_repeats = parser.intval('R', TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS, GRID_MAX_POINTS) + 1);
  #else
    if (parser.seen('R'))
      g26_repeats = parser.has_value() ? parser.value_int() : TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS, GRID_MAX_POINTS) + 1;
    else {
  ...
  do {
    ...
    if (location.valid()) {
      ...
      const bool f = st.y == 0,
                 r = st.x >= TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS_X, GRID_MAX_POINTS_X) - 1,
                 b = st.y >= TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS_Y, GRID_MAX_POINTS_Y) - 1;
