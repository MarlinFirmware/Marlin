...
typedef struct {
  ...
  void connect_neighbor_with_line(const xy_int8_t &p1, int8_t dx, int8_t dy) {
    ...
    if (p2.x < 0 || p2.x >= bedlevel.nr_grid_points.x) return;
    if (p2.y < 0 || p2.y >= bedlevel.nr_grid_points.y) return;
...
void GcodeSuite::G26() {
  ...
  #if HAS_MARLINUI_MENU
    g26_repeats = parser.intval('R', bedlevel.used_points() + 1);
  #else
    if (parser.seen('R'))
      g26_repeats = parser.has_value() ? parser.value_int() : bedlevel.used_points() + 1;
    else {
  ...
  do {
    ...
    if (location.valid()) {
      ...
      const bool f = st.y == 0,
                 r = st.x >= bedlevel.nr_grid_points.x - 1,
                 b = st.y >= bedlevel.nr_grid_points.y - 1;
