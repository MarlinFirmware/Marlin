...
// Deploy certain probes before starting probing
  ...
  else
  {
    ...
    if (DEBUGGING(LEVELING))
      DEBUG_ECHOLNPGM_P(PSTR("Save X"), abl.meshCount.x, SP_Y_STR, abl.meshCount.y, SP_Z_STR, abl.measured_z + abl.Z_offset);
...
#else // !PROBE_MANUALLY
  {
    ...
    for (PR_OUTER_VAR = 0; PR_OUTER_VAR < PR_OUTER_SIZE && !isnan(abl.measured_z); PR_OUTER_VAR++)
    {
      ...
      for (PR_INNER_VAR = inStart; PR_INNER_VAR != inStop; pt_index++, PR_INNER_VAR += inInc)
      {
        ...
        if (abl.verbose_level)
          SERIAL_ECHOLNPGM("Probing mesh point ", pt_index, "/", abl.abl_points, ".");
    ...
    {
      if (abl.verbose_level)
        SERIAL_ECHOLNPGM("Probing point ", i + 1, "/3.");
  ...
  if (!isnan(abl.measured_z))
  {
    ...
    if (!abl.dryrun)
    {
      ...
      if (NEAR(current_position.x, abl.probePos.x - probe.offset_xy.x) && NEAR(current_position.y, abl.probePos.y - probe.offset_xy.y))
      {
        ...
        if (DEBUGGING(LEVELING))
          DEBUG_ECHOLNPGM("Probed Z", simple_z, "  Matrix Z", converted.z, "  Discrepancy ", simple_z - converted.z);
    ...
    {
      if (DEBUGGING(LEVELING))
        DEBUG_ECHOLNPGM("G29 uncorrected Z:", current_position.z);
      ...
      if (DEBUGGING(LEVELING))
        DEBUG_ECHOLNPGM(" corrected Z:", current_position.z);
    }
  ...
  if (DEBUGGING(LEVELING))
    DEBUG_ECHOLNPGM("Z Probe End Script: ", Z_PROBE_END_SCRIPT);
  planner.synchronize();
