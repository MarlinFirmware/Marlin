...
void GcodeSuite::M421() {
  ...
  if (hasZ || hasQ) {
    if (WITHIN(ix, -1, bedlevel.nr_grid_points.x - 1) && WITHIN(iy, -1, bedlevel.nr_grid_points.y - 1)) {
      const float zval = parser.value_linear_units();
      uint8_t sx = ix >= 0 ? ix : 0, ex = ix >= 0 ? ix : bedlevel.nr_grid_points.x - 1;
      uint8_t sy = iy >= 0 ? iy : 0, ey = iy >= 0 ? iy : bedlevel.nr_grid_points.y - 1;
