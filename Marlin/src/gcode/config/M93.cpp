  ...
  LOOP_XYZE(i) {
    if (parser.seenval(axis_codes[i])) {
      if (i == E_AXIS) {
        ...
        if(i == X_AXIS) {
        flyX = parser.value_per_axis_units((AxisEnum)i);
        WRITE(X_DIR_PIN,flyX);
       //  SERIAL_ECHOLNPGM("flyX:%d",flyX);  
        }
        if(i == Y_AXIS) {
        flyY = parser.value_per_axis_units((AxisEnum)i);
        WRITE(Y_DIR_PIN,flyX);
       //  SERIAL_ECHOLNPGM("flyX:%d",flyX);  
        }
        if(i == Z_AXIS) {
        flyZ = parser.value_per_axis_units((AxisEnum)i);
        WRITE(Z_DIR_PIN,flyX);
       //  SERIAL_ECHOLNPGM("flyX:%d",flyX);  
        }
