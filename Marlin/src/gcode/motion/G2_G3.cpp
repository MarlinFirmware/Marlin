...
void plan_arc(
  ...
  float angular_travel = ATAN2(rvec.a * rt_Y - rvec.b * rt_X, rvec.a * rt_X + rvec.b * rt_Y);
    ...
    uint16_t min_segments;
  #else
    constexpr uint16_t min_segments = 1;
  #endif

  // Make a circle if the angular rotation is near 0 and the target is current position or if angular rotation is perfectly 0
  if (!angular_travel || NEAR_ZERO(angular_travel) && NEAR(current_position[p_axis], cart[p_axis]) && NEAR(current_position[q_axis], cart[q_axis])) {
    //even though a circle is a circle we should still probably respect users wish on direction of motion via G2/G3
    angular_travel = (clockwise?-1:1) * RADIANS(360); // clockwise?-RADIANS(360):RADIANS(360) 
    #ifdef MIN_ARC_SEGMENTS
    ...
    #ifdef MIN_ARC_SEGMENTS
      min_segments = CEIL((MIN_ARC_SEGMENTS) * ABS(angular_travel) / RADIANS(360));
