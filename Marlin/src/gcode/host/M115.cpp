...
void GcodeSuite::M115() {
  ...
  #define MACHINE_KINEMATICS "" \
    IF_ENABLED(COREXY, "COREXY") IF_ENABLED(COREYX, "COREYX") \
    IF_ENABLED(COREXZ, "COREXZ") IF_ENABLED(COREZX, "COREZX") \
    IF_ENABLED(COREYZ, "COREYZ") IF_ENABLED(COREZY, "COREZY") \
    IF_ENABLED(MARKFORGED_XY, "MARKFORGED_XY") IF_ENABLED(MARKFORGED_YX, "MARKFORGED_YX") \
    ...
    IF_ENABLED(BELTPRINTER, " BELTPRINTER")
  ...
  SERIAL_EOL();
    ...
    // PAREN_COMMENTS
    IF_ENABLED(PAREN_COMMENTS, cap_line(F("PAREN_COMMENTS")));

    // QUOTED_STRINGS
    IF_ENABLED(GCODE_QUOTED_STRINGS, cap_line(F("QUOTED_STRINGS")));
