...
void GCodeParser::parse(ROString p) {
  ...
  IF_DISABLED(FASTER_GCODE_PARSER, command_args = p.buffer()); // Scan for parameters in seen()
  ...
  while (true) {  // Get the next parameter. A NUL ends the loop
    ...
    if (PARAM_OK(param)) {
      ...
      #if ENABLED(DEBUG_GCODE_PARSER)
        if (debug) {
          SERIAL_ECHOPAIR("Got param ", AS_CHAR(param), " at index ", p.buffer() - command_ptr - 1);
