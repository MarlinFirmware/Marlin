...
void Backlash::add_correction_steps(const xyze_long_t &dist, const AxisBits dm, block_t * const block) {
  ...
  LOOP_NUM_AXES(axis) {
    TERN_(HAS_NONLINEAR_KINEMATICS, sqr_stepper_space_mm += sq(dist[axis] * planner.mm_per_step[axis]));
  ...
  if (changed)
    block->millimeters += TERN(HAS_NONLINEAR_KINEMATICS, millimeters_delta * block->millimeters / sqr_stepper_space_mm, millimeters_delta / block->millimeters);
}
