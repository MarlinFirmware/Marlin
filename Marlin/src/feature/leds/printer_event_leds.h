...
public:
  #if HAS_TEMP_HOTEND
    static LEDColor_t onHotendHeatingStart() { old_intensity = 0; return leds.get_color(); }
    static void onHotendHeating(const celsius_t start, const celsius_t current, const celsius_t target);
  #endif

  #if HAS_HEATED_BED
    static LEDColor_t onBedHeatingStart() { old_intensity = 127; return leds.get_color(); }
    static void onBedHeating(const celsius_t start, const celsius_t current, const celsius_t target);
  #endif

  #if HAS_HEATED_CHAMBER
    static LEDColor_t onChamberHeatingStart() { old_intensity = 127; return leds.get_color(); }
    ...
    static void onPIDTuningDone(LEDColor_t c) { leds.set_color(c); }
  #endif
