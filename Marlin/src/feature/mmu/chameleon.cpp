...
void Chameleon::tool_change(const uint8_t index) {
  const uint8_t switches = motion.extruder ^ index;
  if (!switches) return;

  ui.status_printf(0, GET_TEXT_F(MSG_MMU2_LOADING_FILAMENT), int(index + 1));

  #ifdef CHAMELEON_TOOLCHANGE_Y
    motion.position.y = CHAMELEON_TOOLCHANGE_Y;
  #endif
  #ifdef CHAMELEON_TOOLCHANGE_Z
    NOLESS(motion.position.z, CHAMELEON_TOOLCHANGE_Z);
    motion.goto_current_position(5000);
  #endif
  if (switches & 0x1) {
    motion.position.x = CHAMELEON_BUTTON1_X + (X_CENTER > (CHAMELEON_BUTTON1_X)) ? 10 : -10;
  }
  else {
    #if EXTRUDERS > 2
      if (switches & 0x2)
        motion.position.x = CHAMELEON_BUTTON2_X + (X_CENTER > (CHAMELEON_BUTTON2_X)) ? 10 : -10;
    #endif
  }
  motion.goto_current_position(5000);
...
void Chameleon::execute_extruder_sequence(const E_Step * const sequence, const uint8_t steps) {
  ...
  for (uint8_t i = 0; i < steps; ++i) {
    ...
    if (es) {
      motion.position.e += es;
      motion.goto_current_position(MMM_TO_MMS(fr_or_ms));
      planner.synchronize();
    }
    else
      safe_delay(fr_or_ms);
...
void Chameleon::toggle(const uint8_t switches) {
  if (switches & 0x1) {
    motion.position.x = CHAMELEON_BUTTON1_X;
    motion.goto_current_position(2000);
    motion.position.x += (X_CENTER > (CHAMELEON_BUTTON1_X)) ? 10 : -10;
    motion.goto_current_position(2000);
  }
  #if EXTRUDERS > 2
    if (switches & 0x2) {
      motion.position.x = CHAMELEON_BUTTON2_X;
      motion.goto_current_position(2000);
      motion.position.x += (X_CENTER > (CHAMELEON_BUTTON2_X)) ? 10 : -10;
      motion.goto_current_position(2000);
    }
  #endif
  planner.synchronize();
