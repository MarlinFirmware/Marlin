...
void unified_bed_leveling::shift_mesh_height() {
  ...
  void unified_bed_leveling::probe_entire_mesh(const xy_pos_t &nearby, const bool do_ubl_mesh_map, const bool stow_probe, const bool do_furthest) {
    ...
    do {
      ...
      TERN_(HAS_DISPLAY, ui.status_printf(0, F(S_FMT " %i/%i"), GET_TEXT_F(MSG_PROBING_POINT), point_num, int(GRID_MAX_POINTS)));
  ...
  void unified_bed_leveling::tilt_mesh_based_on_probed_grid(const bool do_3_pt_leveling) {
    ...
    if (do_3_pt_leveling) {
      ...
      for (uint8_t i = 0; i < 3; ++i) {
        ...
        TERN_(HAS_DISPLAY, ui.status_printf(0, F(S_FMT " %i/3"), GET_TEXT_F(MSG_LCD_TILTING_MESH), i + 1));
      ...
      for (uint8_t ix = 0; ix < param.J_grid_size; ++ix) {
        ...
        for (uint8_t iy = 0; iy < param.J_grid_size; ++iy) {
          ...
          TERN_(HAS_DISPLAY, ui.status_printf(0, F(S_FMT " %i/%i"), GET_TEXT_F(MSG_LCD_TILTING_MESH), point_num, total_points));
