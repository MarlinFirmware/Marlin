...
#include "../../../core/debug_out.h"
  ...
  void unified_bed_leveling::line_to_destination_cartesian(const_feedRate_t scaled_fr_mm_s, const uint8_t extruder) {
    ...
    if (istart == iend) {
      ...
      const float xratio = (end.x - get_mesh_x(iend.x)) * TERN(VARIABLE_GRID_POINTS, mesh_dist_recip.x, RECIPROCAL(MESH_X_DIST)),
                  yratio = (end.y - get_mesh_y(iend.y)) * TERN(VARIABLE_GRID_POINTS, mesh_dist_recip.y, RECIPROCAL(MESH_Y_DIST)),
  ...
  bool __O2 unified_bed_leveling::line_to_destination_segmented(const_feedRate_t scaled_fr_mm_s) {
    ...
    for (;;) {  // for each mesh cell encountered during the move
      ...
      xy_int8_t icell = {
        int8_t((raw.x - (MESH_MIN_X)) * TERN(VARIABLE_GRID_POINTS, mesh_dist_recip.x, RECIPROCAL(MESH_X_DIST))),
        int8_t((raw.y - (MESH_MIN_Y)) * TERN(VARIABLE_GRID_POINTS, mesh_dist_recip.y, RECIPROCAL(MESH_Y_DIST)))
      };
      LIMIT(icell.x, 0, TERN(VARIABLE_GRID_POINTS, GRID_USED_CELLS_X, GRID_MAX_CELLS_X));
      LIMIT(icell.y, 0, TERN(VARIABLE_GRID_POINTS, GRID_USED_CELLS_Y, GRID_MAX_CELLS_Y));

      const int8_t ncellx = _MIN(icell.x + 1, TERN(VARIABLE_GRID_POINTS, GRID_USED_CELLS_X, GRID_MAX_CELLS_X)),
                   ncelly = _MIN(icell.y + 1, TERN(VARIABLE_GRID_POINTS, GRID_USED_CELLS_Y, GRID_MAX_CELLS_Y));
      ...
      const float z_xmy0 = (z_x1y0 - z_x0y0) * TERN(VARIABLE_GRID_POINTS, mesh_dist_recip.x, RECIPROCAL(MESH_X_DIST)), // z slope per x along y0 (lower left to lower right)
                  z_xmy1 = (z_x1y1 - z_x0y1) * TERN(VARIABLE_GRID_POINTS, mesh_dist_recip.x, RECIPROCAL(MESH_X_DIST)); // z slope per x along y1 (upper left to upper right)
      ...
      const float z_cxy1 = z_x0y1 + z_xmy1 * cell.x,    // z height along y1 at cell.x
            ...
            float z_cxym = z_cxyd * TERN(VARIABLE_GRID_POINTS, mesh_dist_recip.y, RECIPROCAL(MESH_Y_DIST));  // z slope per y along cell.x from pos.y to y1 (changes for each cell.x in cell)
      ...
      const float z_sxy0 = z_xmy0 * diff.x,                                 // per-segment adjustment to z_cxy0
                  z_sxym = (z_xmy1 - z_xmy0) * TERN(VARIABLE_GRID_POINTS, mesh_dist_recip.y, RECIPROCAL(MESH_Y_DIST)) * diff.x;  // per-segment adjustment to z_cxym
      ...
      for (;;) {  // for all segments within this mesh cell
        ...
        if (!WITHIN(cell.x, 0, TERN(VARIABLE_GRID_POINTS, mesh_dist.x, MESH_X_DIST)) || !WITHIN(cell.y, 0, TERN(VARIABLE_GRID_POINTS, mesh_dist.y, MESH_Y_DIST)))    // done within this cell, break to next
          break;
