...
#include "../../../core/debug_out.h"
  ...
  void unified_bed_leveling::line_to_destination_cartesian(const feedRate_t scaled_fr_mm_s, const uint8_t extruder) {
    ...
    if (istart == iend) {
      ...
      const float xratio = (end.x - get_mesh_x(iend.x)) * GRID_VAL(mesh_dist_recip.x, RECIPROCAL(MESH_X_DIST)),
                  yratio = (end.y - get_mesh_y(iend.y)) * GRID_VAL(mesh_dist_recip.y, RECIPROCAL(MESH_Y_DIST)),
  ...
  bool __O2 unified_bed_leveling::line_to_destination_segmented(const feedRate_t scaled_fr_mm_s) {
    ...
    for (;;) {  // for each mesh cell encountered during the move
      ...
      xy_int8_t icell = {
        int8_t((raw.x - (MESH_MIN_X)) * GRID_VAL(mesh_dist_recip.x, RECIPROCAL(MESH_X_DIST))),
        int8_t((raw.y - (MESH_MIN_Y)) * GRID_VAL(mesh_dist_recip.y, RECIPROCAL(MESH_Y_DIST)))
      };
      LIMIT(icell.x, 0, GRID_PREF_CELLS_X);
      LIMIT(icell.y, 0, GRID_PREF_CELLS_Y);
      ...
      for (;;) {  // for all segments within this mesh cell
        ...
        if (!WITHIN(cell.x, 0, GRID_VAL(mesh_dist.x, MESH_X_DIST)) || !WITHIN(cell.y, 0, GRID_VAL(mesh_dist.y, MESH_Y_DIST)))    // done within this cell, break to next
          break;
