...
constexpr int8_t  to_fix(const int8_t  v) { return v * 2; }
constexpr int8_t  to_int(const int8_t  v) { return v / 2; }
constexpr uint8_t   log2(const uint8_t n) { return (n > 1) ? 1 + log2(uint8_t(n >> 1)) : 0; }
constexpr uint8_t  order(const uint8_t n) { return uint8_t(log2(uint8_t(n - 1))) + 1; }
#if ENABLED(VARIABLE_GRID_POINTS)
  #define ord order(_MAX(bedlevel.nr_grid_points.x, bedlevel.nr_grid_points.y))
  #define dim _BV(ord)
#else
  constexpr uint8_t ord = order(_MAX(bedlevel.nr_grid_points.x, bedlevel.nr_grid_points.y));
...
static inline bool eval_candidate(int8_t x, int8_t y, hilbert_curve::callback_ptr func, void *data) {
  ...
  return x < bedlevel.nr_grid_points.x && y < bedlevel.nr_grid_points.y ? func(x, y, data) : false;
}
...
bool hilbert_curve::search_from_closest(const xy_pos_t &pos, hilbert_curve::callback_ptr func, void *data) {
  ...
  return search_from(bedlevel.cell_index_x(pos.x), bedlevel.cell_index_y(pos.y), func, data);
}
