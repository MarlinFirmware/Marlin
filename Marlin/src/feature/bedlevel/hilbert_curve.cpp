...
bool hilbert_curve::search_from_closest(const xy_pos_t &pos, hilbert_curve::callback_ptr func, void *data) {
  ...
  const uint8_t grid_x = LROUND(constrain((pos.x - mesh_min.x) / GRID_VAL(bedlevel.mesh_dist.x, MESH_X_DIST), 0, GRID_PREF_POINTS_X - 1));
  const uint8_t grid_y = LROUND(constrain((pos.y - mesh_min.y) / GRID_VAL(bedlevel.mesh_dist.y, MESH_Y_DIST), 0, GRID_PREF_POINTS_Y - 1));
