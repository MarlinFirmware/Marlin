  ...
  if (force
    ...
    void PrintJobRecovery::retract_and_lift(const_float_t zraise) {
        ...
        gcode.set_relative_mode(true);  // Use relative coordinates
          ...
          if (zraise) gcode.process_subcommands_now(DString::format(F("G0Z%1.3f"), zraise));
        #else
          UNUSED(zraise);
        #endif
...
void PrintJobRecovery::resume() {
  ...
  TERN_(PREVENT_COLD_EXTRUSION, thermalManager.allow_cold_extrude = info.flag.allow_cold_extrusion);
    ...
    if (bt) {
      ...
      gcode.process_subcommands_now(DString::format(F("M190S%i"), bt));
    }
  #endif
  ...
  // Heat hotend enough to soften material
    ...
    HOTEND_LOOP() {
      ...
      if (et) {
        #if HAS_MULTI_HOTEND
          gcode.process_subcommands_now(DString::format(F("T%iS"), e));
        #endif
  ...
  gcode.process_subcommands_now_P(PSTR("G92.9E0")); // Reset E to 0
    ...
    gcode.process_subcommands_now(DString::format(F(
    ...
    #if HOME_XY_ONLY
      gcode.process_subcommands_now(DString::format(F("G92.9Z%s"), z_now));
    #endif
    ...
    gcode.process_subcommands_now(DString::format(F("G1X%1.3fY%1.3fF1000\nG28Z"), p.x, p.y));
  #endif
  ...
  set_all_homed();
    ...
    gcode.process_subcommands_now(DString::format(F("M420S%iZ%1.3f"), int(info.flag.leveling), info.fade));
  ...
  // Recover volumetric extrusion state
      ...
      for (int8_t e = 0; e < EXTRUDERS; e++) {
        gcode.process_subcommands_now(DString::format(F("M200T%iD%1.3f"), e, info.filament_size[e]));
      }
      if (!info.flag.volumetric_enabled) {
        gcode.process_subcommands_now(DString::format(F("M200T%iD0"), info.active_extruder));
      }
    #else
      if (info.flag.volumetric_enabled) {
        gcode.process_subcommands_now(DString::format(F("M200D%1.3f"), info.filament_size[0]));
      }
    #endif
  #endif
  ...
  // Restore the previously active tool (with no_move)
    ...
    gcode.process_subcommands_now(DString::format(F("T%iS"), info.active_extruder));
  #endif
  ...
  // Restore print cooling fan speeds
    ...
    FANS_LOOP(i) {
      ...
      if (f) gcode.process_subcommands_now(DString::format(F("M106P%iS%i"), i, f));
    }
  #endif
  ...
  gcode.process_subcommands_now(DString::format(F("G1X%1.3fY%1.3fF3000"), info.current_position.x, info.current_position.y));

  // Move back down to the saved Z for printing
  gcode.process_subcommands_now(DString::format(F("G1Z%1.3fF600"), info.current_position.z));

  // Restore the feedrate
  gcode.process_subcommands_now(DString::format(F("G1F%d"), info.feedrate));

  // Restore E position with G92.9
  gcode.process_subcommands_now(DString::format(F("G92.9E%1.3f"), info.current_position.e));
