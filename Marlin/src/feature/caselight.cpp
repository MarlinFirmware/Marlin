...
bool CaseLight::on = CASE_LIGHT_DEFAULT_ON;
  ...
  LEDColor_t CaseLight::color = { init_case_light[0], init_case_light[1], init_case_light[2] OPTARG(HAS_WHITE_LED, init_case_light[3]) };
#endif
...
void CaseLight::update(const bool sflag) {
    ...
    UNUSED(n10ct);
      ...
      if (on)
        ...
        leds.set_color(LEDColor_t(leds.color.r, leds.color.g, leds.color.b OPTARG(HAS_WHITE_LED, leds.color.w) OPTARG(NEOPIXEL_LED, n10ct)));
      else
        // Switch off leds
        leds.set_off();
    #else
      // Use CaseLight color (CASE_LIGHT_DEFAULT_COLOR) and new brightness level
      leds.set_color(LEDColor_t(color.r, color.g, color.b OPTARG(HAS_WHITE_LED, color.w) OPTARG(NEOPIXEL_LED, n10ct)));
    #endif
