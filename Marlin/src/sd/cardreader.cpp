...
MarlinFile CardReader::root, CardReader::workDir, CardReader::workDirParents[MAX_DIR_DEPTH];
...
//
int CardReader::countItems(MarlinFile dir) {
  dir_t p;
  int c = 0;
...
//
void CardReader::selectByIndex(MarlinFile dir, const uint8_t index) {
  dir_t p;
...
//
void CardReader::selectByName(MarlinFile dir, const char * const match) {
  dir_t p;
...
void CardReader::printListing(
  MarlinFile parent, const char * const prepend
  ...
  while (parent.readDir(&p, longFilename) > 0) {
    if (DIR_IS_SUBDIR(&p)) {
      ...
      MarlinFile child; // child.close() in destructor
...
void CardReader::ls(
  ...
  void CardReader::printLongPath(char * const path) {
    ...
    MarlinFile diveDir = root; // start from the root for segment 1
    for (i = 0; i < pathLen;) {
...
void CardReader::getAbsFilenameInCWD(char *dst) {
  *dst++ = '/';
  ...
  auto appendAtom = [&](MarlinFile &file) {
    file.getDosName(dst);
 ...
 *
 * On exit:
 *  - Your curDir pointer contains an MarlinFile reference to the file's directory.
 ...
 * On exit, inDirPtr contains an MarlinFile reference to the file's directory.
 *
 ...
 */
const char* CardReader::diveToFile(const bool update_cwd, MarlinFile* &inDirPtr, const char * const path, const bool echo/*=false*/) {
  DEBUG_SECTION(est, "diveToFile", true);

  // Track both parent and subfolder
  static MarlinFile newDir1, newDir2;
  MarlinFile *sub = &newDir1, *startDirPtr;
...
void CardReader::cd(const char * relpath) {
  MarlinFile newDir, *parent = &getWorkDir();
