...
int8_t SdBaseFile::readDir(dir_t* dir, char* longFilename) {
...
#else
  #include "..\longer\unicode2gbk.h"
  int8_t SdBaseFile::readDir(dir_t* dir, char* longFilename) 
  {
  int16_t n;
  unsigned short wTemp[3 * 13 + 1] ={0};
  ...
  while (1) {
    ...
    if (longFilename && DIR_IS_LONG_NAME(dir)) {
      ...
      if (VFAT->firstClusterLow == 0) {
        ...
        if (WITHIN(seq, 1, MAX_VFAT_ENTRIES)) {
          ...
          n = (seq - 1) * (FILENAME_LENGTH);
          for (uint8_t i = 0; i < FILENAME_LENGTH; i++){
            wTemp[n + i] = (i < 5) ? VFAT->name1[i] : (i < 11) ? VFAT->name2[i - 5] : VFAT->name3[i - 11];
            //MYSERIAL0.println(w);
          }
          // If this VFAT entry is the last one, add a NUL terminator at the end of the string
          if (VFAT->sequenceNumber & 0x40) wTemp[n + FILENAME_LENGTH] = 0;
        }
      #endif
      }
    }
    ...
    if (DIR_IS_FILE_OR_SUBDIR(dir)) {
      unsigned short w, *lfn;
      int i = 0;
      lfn = wTemp;
      while ((w = *lfn++) != 0) {			/* Get an LFN char */
				w = ff_convert(w, 0);			/* Unicode -> OEM conversion */
				if (!w) { i = 0; break; }		/* Could not convert, no LFN */
				if (w >= 0x100)		/* Put 1st byte if it is a DBC (always false on SBCS cfg) */
					longFilename[i++] = (unsigned char)(w >> 8);
				if (i >= LONG_FILENAME_LENGTH - 1) { i = 0; break; }	/* Buffer overflow, no LFN */
				longFilename[i++] = (unsigned char)w;
			}
      longFilename[i] = '\0';	/* Terminate the LFN str by a \0 */
      return n;
      }
  }
}
