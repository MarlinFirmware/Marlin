...
#include "gcode/queue.h"
  ...
  #include "libs/L6470/L6470_Marlin.h"
   ...
   #include "../../lcd/menu/touch/ili9341.h"
   #include "../../longer/LGT_MACRO.h"
   #include "../../longer/config_storedata.h"
   display_image Display_Screen;
   ...
   void LGT_Lcdfsmc_init();
   #define INIT_FIL_RUNOUT_PIN(p)  SET_INPUT(p)
  #endif
bool Running = true;
...
void idle(
  ...
  #ifdef LONGER3D
  Display_Screen.LGT_Main_Function();
  #else
  ui.update();
  #endif
...
void kill(PGM_P const lcd_msg/*=nullptr*/) {
  #ifdef LONGER3D
  Display_Screen.LGT_Printerabnormal_Kill(lcd_msg ? lcd_msg : PSTR(MSG_KILLED));
...
//TOUCH_LCD_BacklightOff();
  ...
  #endif
#ifdef LONGER3D
  W25QxxFlash.W25QXX_Init();
  ConfigSettings.settingsLoad();
  W25QxxFlash.W25QXX_Read((uint8_t*)&flashsave,SAVE_BASE_ADDR,sizeof(SAVEDATA));
  xCalibration=flashsave.s_result[0];
  xOffset=flashsave.s_result[1];
  yCalibration=flashsave.s_result[2];
  yOffset=flashsave.s_result[3];
  // Serial1.println(flashsave.checksflag);
  // Serial1.println(xCalibration);
  // Serial1.println(xOffset);
  // Serial1.println(yCalibration);
  // Serial1.println(yOffset);
   INIT_FIL_RUNOUT_PIN(FIL_RUNOUT_PIN);   
   Display_Screen.LGT_lcd_init();
   Display_Screen.displayStartUpLogo();
   delay(1000);
   #endif
}
