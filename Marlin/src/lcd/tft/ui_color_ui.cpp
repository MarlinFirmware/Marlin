...
void MenuItem_confirm::draw_select_screen(FSTR_P const yes, FSTR_P const no, const bool yesno, FSTR_P const fpre, const char * const string/*=nullptr*/, FSTR_P const fsuf/*=nullptr*/) {
  ...
  void MarlinUI::ubl_plot(const uint8_t x_plot, const uint8_t y_plot) {
    ...
    for (uint16_t x = 0; x < bedlevel.nr_grid_points.x; x++)
      for (uint16_t y = 0; y < bedlevel.nr_grid_points.y; y++)
        if (motion.can_reach({ bedlevel.get_mesh_x(x), bedlevel.get_mesh_y(y) }))
          tft.add_bar(1 + (x * 2 + 1) * (UBL_GRID_W - 4) / bedlevel.nr_grid_points.x / 2, UBL_GRID_H - 3 - ((y * 2 + 1) * (UBL_GRID_H - 4) / bedlevel.nr_grid_points.y / 2), 2, 2, COLOR_UBL);

    tft.add_rectangle((x_plot * 2 + 1) * (UBL_GRID_W - 4) / bedlevel.nr_grid_points.x / 2 - 1, UBL_GRID_H - 5 - ((y_plot * 2 + 1) * (UBL_GRID_H - 4) / bedlevel.nr_grid_points.y / 2), 6, 6, COLOR_UBL);
    ...
    tft.add_text(tft_string.center(UBL_TEXT_AREA_W), MENU_TEXT_Y, COLOR_MENU_VALUE, tft_string);
      ...
      touch.clear();
      draw_menu_navigation = false;
      add_control(UBL_GRID_X + UBL_GRID_W + UBL_CONTROL_OFFSET,      UBL_GRID_Y + UBL_CONTROL_OFFSET,                   UBL,  (ENCODER_STEPS_PER_MENU_ITEM) * bedlevel.nr_grid_points.x, imgUp);
      add_control(UBL_GRID_X + UBL_GRID_W + UBL_CONTROL_OFFSET,      UBL_GRID_Y + UBL_GRID_H - UBL_CONTROL_OFFSET - 32, UBL, -(ENCODER_STEPS_PER_MENU_ITEM) * bedlevel.nr_grid_points.x, imgDown);
