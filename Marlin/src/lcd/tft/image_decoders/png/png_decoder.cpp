...
bool PNGDecoder::decode(const uint8_t* png_data, size_t png_size,
                        uint16_t* output_buffer, uint16_t width, uint16_t height) {

    DEBUG_ECHOLN("PNGDecoder::decode: Starting PNG decode with stb_image, size=", png_size, ", expected dims=", width, "x", height);

    int img_width, img_height, channels;

    // Decode PNG - request RGB or RGBA
    unsigned char* img_data = stbi_load_from_memory(png_data, png_size,
                                                     &img_width, &img_height, &channels, 0);

    if (!img_data) {
        DEBUG_ECHOLN("PNGDecoder::decode: stbi_load_from_memory failed");
        return false;
    }

    DEBUG_ECHOLN("PNGDecoder::decode: PNG decoded successfully, actual dims=", img_width, "x", img_height, ", channels=", channels);

    // Check dimensions match expected
    if (img_width != width || img_height != height) {
        DEBUG_ECHOLN("PNGDecoder::decode: Dimension mismatch: got ", img_width, "x", img_height, ", expected ", width, "x", height);
        stbi_image_free(img_data);
        return false;
    }

    DEBUG_ECHOLN("PNGDecoder::decode: Converting to RGB565, channels=", channels);
    ...
    DEBUG_ECHOLN("PNGDecoder::decode: RGB conversion completed successfully");
    stbi_image_free(img_data);
    DEBUG_ECHOLN("PNGDecoder::decode: PNG decode completed successfully");
    return true;
}

bool PNGDecoder::getDimensions(const uint8_t* png_data, size_t png_size,
    ...
    if (!result) {
        DEBUG_ECHOLN("PNGDecoder::getDimensions: stbi_info_from_memory failed");
        return false;
    }
    ...
    DEBUG_ECHOLN("PNGDecoder::getDimensions: PNG dimensions: ", width, "x", height, ", channels=", channels);
    return true;
}
