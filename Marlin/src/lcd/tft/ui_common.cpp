...
void MenuItem_static::draw(const uint8_t row, FSTR_P const ftpl, const uint8_t style/*=SS_DEFAULT*/, const char *vstr/*=nullptr*/) {
  ...
  if (!full || !vstr) {
    ...
    tft.add_text(center ? tft_string.center(TFT_WIDTH) : MENU_TEXT_X, MENU_TEXT_Y, color, tft_string);
    return;
  }
  ...
  tft.add_text(MENU_TEXT_X, MENU_TEXT_Y, color, tft_string);

  // Right-justified value, after spaces
  while (*vstr == ' ') vstr++;
  tft_string.set(vstr);
  tft.add_text(TFT_WIDTH - MENU_TEXT_X - 1 - tft_string.width(), MENU_TEXT_Y, color, tft_string);
}
