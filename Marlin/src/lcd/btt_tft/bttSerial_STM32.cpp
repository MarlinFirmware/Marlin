...
BttSerial BttSerial1(UART);


void BttSerial::setRx(uint32_t _rx) { _serial.pin_rx = digitalPinToPinName(_rx); }
void BttSerial::setTx(uint32_t _tx) { _serial.pin_tx = digitalPinToPinName(_tx); }
void BttSerial::setRx(PinName _rx)  { _serial.pin_rx = _rx; }
void BttSerial::setTx(PinName _tx)  { _serial.pin_tx = _tx; }

void BttSerial::init(PinName _rx, PinName _tx) {
  _serial.pin_rx  = (_rx == _tx) ? NC : _rx;
  _serial.pin_tx  = _tx;
  _serial.rx_buff = bttRxBuf;
  _serial.rx_head = 0;
  _serial.rx_tail = 0;
  _serial.tx_buff = bttTxBuf;
  _serial.tx_head = 0;
  _serial.tx_tail = 0;
}

BttSerial::BttSerial(void *peripheral) {
...
bool BttSerial::isHalfDuplex() const { return _serial.pin_rx == NC; }

void BttSerial::enableHalfDuplexRx() {
  if (isHalfDuplex()) {
...
void BttSerial::_rx_complete_irq(serial_t *obj) {
  ...
  if (uart_getc(obj, &c) == 0) {
    ...
    rx_buffer_index_t i = (unsigned int)(obj->rx_head + 1) % BTT_TFT_RX_BUF_SIZE;
...
int BttSerial::_tx_complete_irq(serial_t *obj) {
  // If interrupts are enabled, there must be more data in the output
  // buffer. Send the next byte
  obj->tx_tail = (obj->tx_tail + 1) % BTT_TFT_TX_BUF_SIZE;

  return (obj->tx_head == obj->tx_tail) ? -1 : 0;
}

void BttSerial::begin(unsigned long baud) { begin(baud, SERIAL_8N1); }

void BttSerial::begin(unsigned long baud, byte config) {
...
int BttSerial::available() {
  return ((unsigned int)(BTT_TFT_RX_BUF_SIZE + _serial.rx_head - _serial.rx_tail)) % BTT_TFT_RX_BUF_SIZE;
}
...
int BttSerial::read() {
  ...
  _serial.rx_tail = (rx_buffer_index_t)(_serial.rx_tail + 1) % BTT_TFT_RX_BUF_SIZE;
  return c;
}

int BttSerial::write(uint8_t c) {
  _written = true;
  if (isHalfDuplex()) {
    if (_rx_enabled) {
      _rx_enabled = false;
  ...
  tx_buffer_index_t i = (_serial.tx_head + 1) % BTT_TFT_TX_BUF_SIZE;
