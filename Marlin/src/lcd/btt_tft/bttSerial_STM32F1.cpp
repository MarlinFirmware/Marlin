...
DEFINE_WFSERIAL(BttSerial1, UART_NUMMER);

BttSerial::BttSerial(usart_dev *usart_device, uint8 tx_pin, uint8 rx_pin) {
...
void BttSerial::begin(uint32 baud) { begin(baud, SERIAL_8N1); }
...
void BttSerial::begin(uint32 baud, uint8_t config) {
  ...
  rb_init(this->usart_device->rb, BTT_TFT_RX_BUF_SIZE, bttRxBuf);
...
int BttSerial::write(unsigned char ch) {
  usart_putc(this->usart_device, ch);
  return 1;
}

int BttSerial::wifi_rb_is_full() {
