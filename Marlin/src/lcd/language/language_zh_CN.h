...
namespace LanguageNarrow_zh_CN {
  ...
  LSTR MSG_CONTROL_RETRACTF               = _UxGT("回抽速率mm/s"); // "Retract V" retract_feedrate, feedrate for retracting (mm/s)
  LSTR MSG_CONTROL_RETRACT_ZHOP           = _UxGT("Hop mm"); // "Hop mm" retract_zraise, retract Z-lift
  LSTR MSG_CONTROL_RECOVER                = _UxGT("回抽恢复长度mm"); // "UnRet +mm" recover_extra, additional recover length (mm, added to retract length when recovering)
  LSTR MSG_CONTROL_RECOVER_SWAP           = _UxGT("换手回抽恢复长度mm"); // "S UnRet+mm" swap_recover_extra, additional swap recover length (mm, added to retract length when recovering from extruder change)
  LSTR MSG_CONTROL_RECOVERF               = _UxGT("回抽恢复后进料速率mm/s"); // "Unretract V" recover_feedrate, feedrate for recovering from retraction (mm/s)
