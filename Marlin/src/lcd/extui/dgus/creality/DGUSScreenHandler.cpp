...
void DGUSScreenHandlerCreality::sendAboutFirmwareVersion(DGUS_VP_Variable &var) {
  ...
  dgus.writeVariable_P(var.VP, fwVersion, strlen(fwVersion), true);
}
...
void DGUSScreenHandlerCreality::sendAboutPrintSize(DGUS_VP_Variable &var) {
  ...
  dgus.writeVariable_P(var.VP, &PRINTSIZE, sizeof(PRINTSIZE), true);
}
...
void DGUSScreenHandler::handleManualMove(DGUS_VP_Variable &var, void *val_ptr) {
  ...
  if (!movevalue) {
    ...
    // movement
    bool old_relative_mode = motion.relative_mode;
    if (!motion.relative_mode) queue.enqueue_now_P(PSTR("G91"));
    char buf[32];  // G1 X9999.99 F12345
    unsigned int backup_speed = MMS_TO_MMM(motion.feedrate_mm_s);
    char sign[]="\0";
...
bool DGUSScreenHandler::loop() {
  ...
  if (marlin.wait_for_user && current_screenID != DGUS_SCREEN_POPUP) {
