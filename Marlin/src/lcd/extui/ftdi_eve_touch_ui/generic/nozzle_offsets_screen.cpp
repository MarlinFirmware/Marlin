...
void NozzleOffsetScreen::onRedraw(draw_mode_t what) {
  ...
  w.color(Theme::x_axis).adjuster(2,  GET_TEXT_F(MSG_AXIS_X), ExtUI::getToolOffset_mm(X, E1));
  w.color(Theme::y_axis).adjuster(4,  GET_TEXT_F(MSG_AXIS_Y), ExtUI::getToolOffset_mm(Y, E1));
  w.color(Theme::z_axis).adjuster(6,  GET_TEXT_F(MSG_AXIS_Z), ExtUI::getToolOffset_mm(Z, E1));
...
bool NozzleOffsetScreen::onTouchHeld(uint8_t tag) {
  ...
  switch (tag) {
    ...
    case  3: UI_INCREMENT(ToolOffset_mm, X, E1); break;
    case  4: UI_DECREMENT(ToolOffset_mm, Y, E1); break;
    case  5: UI_INCREMENT(ToolOffset_mm, Y, E1); break;
    case  6: UI_DECREMENT(ToolOffset_mm, Z, E1); break;
    case  7: UI_INCREMENT(ToolOffset_mm, Z, E1); break;
    #if ENABLED(CALIBRATION_GCODE)
