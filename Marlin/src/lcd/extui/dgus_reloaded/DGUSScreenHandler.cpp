...
void DGUSScreenHandler::meshUpdate(const int8_t xpos, const int8_t ypos) {
  ...
  uint8_t point = ypos * TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS_X, GRID_MAX_POINTS_X) + xpos;
  probing_icons[point < 16 ? 0 : 1] |= (1U << (point % 16));

  if (xpos >= TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS_X, GRID_MAX_POINTS_X) - 1 && ypos >= TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS_Y, GRID_MAX_POINTS_Y) - 1 && !ExtUI::getLevelingIsValid())
