...
public:
  ...
  void Startup();
  void Idle();
  void PrinterKilled(PGM_P const error, PGM_P const component);
  void MediaInserted();
  void MediaError();
  void MediaRemoved();
  void PrintTimerStarted(bool sd);
  void PrintTimerStopped();
  void PrintFinished();
  void FilamentRunout();
  void UserConfirmRequired(const char * const msg);  
  void StatusChanged(const char * const text);
  void HomingStart();
  void HomingComplete();
  void Popup_Temperature(const bool toohigh);
