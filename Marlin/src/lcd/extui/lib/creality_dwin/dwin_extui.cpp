...
/**
 * DWIN by Creality3D
 * Extui Port by Miguel A. Risco-Castillo
 */
...
void EachMomentUpdate() {
  ...
  if (checkkey == PrintProcess) { // print process
    uint8_t percentDone = sdprint ? card.percentDone() : (ui._get_progress()/100);
...
void CrealityDWINClass::Idle() {
  EachMomentUpdate();   // Status update
  DWIN_HandleScreen();  // Rotary encoder update
}

void CrealityDWINClass::PrinterKilled(PGM_P const error, PGM_P const component) {
  Draw_Popup_Confirm(ICON_BLTouch,"Printer Killed", error);
}

void CrealityDWINClass::MediaInserted() { 
  card.cdroot();
  if (checkkey == SelectFile) Redraw_SD_List();   
}

void CrealityDWINClass::MediaError(){};

void CrealityDWINClass::MediaRemoved(){
...
// Start a Print Job
void CrealityDWINClass::PrintTimerStarted(bool sd) {
  Start_Print(sd);
}

//End print job
void CrealityDWINClass::PrintTimerStopped() {
  Stop_Print();
}

void CrealityDWINClass::PrintFinished(){
}

void CrealityDWINClass::FilamentRunout() {
  Draw_Popup_Confirm(ICON_BLTouch,"Filament Runout", "Reload the filament");
}

void CrealityDWINClass::UserConfirmRequired(const char * const msg) {
  Draw_Popup_Confirm(ICON_BLTouch, msg, "");
}

void CrealityDWINClass::StatusChanged(const char * const text) {
...
void CrealityDWINClass::Popup_Temperature(const bool toohigh)
{
  Clear_Popup_Area();
  Draw_Popup_Bkgd_105();
  if (toohigh)
