...
namespace Anycubic {
  ...
  void ChironTFT::PrinterKilled(PGM_P error,PGM_P component)  {
    ...
    #if ACDEBUG(16)
      _SEP_4("PrinterKilled()\nerror: ", error , "\ncomponent: ", component);
    #endif
  }
  void ChironTFT::MediaEvent(media_event_t event)  {
    #if ACDEBUG(16)
      _SELP_2_P(PSTR("ProcessMediaStatus() "),event);
    #endif
    switch(event) {
  ...
  void ChironTFT::TimerEvent(timer_event_t event)  {
    #if ACDEBUG(16)
      _SELP_2_P(PSTR("TimerEvent() "),event);
      _SELP_2_P(PSTR("Printer State: "),printer_state);
    #endif
  ...
  void ChironTFT::FilamentRunout()  {
    #if ACDEBUG(16)
      _SELP_2_P(PSTR("FilamentRunout() printer_state "),printer_state);
    #endif
    // 1 Signal filament out
    if(isPrintingFromMedia()) {
      SendtoTFTLN(AC_msg_filament_out_alert);
    }
    else {
      SendtoTFTLN(AC_msg_filament_out_block);
    }
//  printer_state = AC_printer_filament_out;
  ...
  void ChironTFT::ConfirmationRequest(const char * const msg)  {
    // M108 continue
    #if ACDEBUG(16)
      _SELP_4_P(PSTR("ConfirmationRequest() "), msg, PSTR(" printer_state:"), printer_state );
    #endif
  ...
  void ChironTFT::StatusChange(const char * const msg)  {
    #if ACDEBUG(16)
      _SELP_2_P(PSTR("StatusChange() "),msg);
  ...
  void ChironTFT::PowerLossRecovery()  {
    ...
    // Play tune to notify user we can recover.
  ...
  void ChironTFT::InjectCommandandWait(PGM_P cmd) {
    //injectCommands_P(cmnd);
    queue.enqueue_now_P(cmd);
    //SERIAL_ECHOLN(PSTR("Inject>"));
  }
  void ChironTFT::ProcessPanelRequest() {
    // Break these up into logical blocks
    // as its easier to navigate than one huge switch case!
  ...
  void ChironTFT::PanelInfo(uint8_t req) {
    ...
    switch(req) {
      ...
      case 6: { //A6 Get printing progress 
        ...
        else SendtoTFTLN(PSTR("A6V ---"));
      } break;
      case 7: { //A7 Get Printing Time 
        uint32_t time = 0;
        time = getProgress_seconds_elapsed()/60;
        SendtoTFT(PSTR("A7V "));
        TFTSer.print( itostr2((time-time%60)/60) );
        SendtoTFT(PSTR(" H "));      
        TFTSer.print( itostr2(time%60) );
        SendtoTFT(PSTR(" M"));
        #if ACDEBUG(64)
          _SELP_4_P(PSTR("Print time "), itostr2((time-time%60)/60), PSTR(":"), itostr2(time%60));
        #endif
      } break; 
  ...
  void ChironTFT::PanelAction(uint8_t req) {
    switch(req)
    {
      ...
      case 16: { //A16 Set HotEnd temp  A17 S170
        float set_Htemp = 0;
        set_Htemp = atof(&panel_command[5]);
        if(set_Htemp == 0) {
          hotend_state = AC_heater_off;
        }
        else {
        ...
        switch((char)panel_command[4]) {
          ...
          case 'C':
          {
            setTargetTemp_celsius(set_Htemp, E0);
          }
          break;
        }
      } break;
      case 17: { //A17 Set bed temp
        float set_Btemp = 0;
        set_Btemp = atof(&panel_command[5]);
        if(set_Btemp == 0) {
          hotbed_state = AC_heater_off;
        }
        else {
      ...
      case 22: { //A22 Move Axis  A22 Y +10F3000
        ...
        if(!isPrinting()) {
          ...
          #if ACDEBUG(2)
            _SELP_4_P(PSTR("Nudge "),panel_command[4], PSTR(" axis ") , newposition );
		      #endif
        
		      switch(panel_command[4]) {
            case 'X':
		          setAxisPosition_mm(getAxisPosition_mm(X)+newposition,X);
            break;
		        case 'Y':
		          setAxisPosition_mm(getAxisPosition_mm(Y)+newposition,Y);
		        break;
			      case 'Z':
			        setAxisPosition_mm(getAxisPosition_mm(Z)+newposition,Z);
			      break;
    ...
    switch(req)
    {
      case 29: { //A29 Read Mesh Point A29 X1 Y1 
        ...
        if(!isPrinting()) {
          ...
          // If the same meshpoint is selected twice in a row, move the head to that ready for adjustment
            ...
            if(isPositionKnown()) { 
              #if ACDEBUG(1)
                _SELP_6_P(PSTR("Moving to mesh point at x: "),pos.x,PSTR(" y: "),pos.y,PSTR(" z: "),pos_z);
              #endif
              // Go up before moving
              ...
              #if ACDEBUG(1)
		            _SELP_2_P(PSTR("Current Z: "),getAxisPosition_mm(Z));
              #endif
            } 
          }
      ...
      case 31: { //A31 Adjust all Probe Points
        switch(panel_command[3]) {
          ...
          case 'S': { // Set offset (adjusts all points by value)
            ...
            if(isPrinting()) {
              #if ACDEBUG(1) 
                _SELP_4_P(PSTR("Change Zoffset from:"), live_Zoffset, PSTR(" to "), live_Zoffset+Zshift);
              #endif
              if(isAxisPositionKnown(Z)) {
                
                #if ACDEBUG(1) 
                  float currZpos = getAxisPosition_mm(Z);
                  _SELP_4_P(PSTR("Nudge Z pos from "), currZpos ,PSTR(" to "), currZpos+constrain(Zshift,-0.05,0.05) );
                #endif 
                // Use babystepping to adjust the head position
                int16_t steps = mmToWholeSteps(constrain(Zshift,-0.05,0.05), Z);
                #if ACDEBUG(1) 
                  _SELP_2_P(PSTR("Steps to move Z: "),steps);
                #endif
              ...
              for(uint8_t x = 0; x < GRID_MAX_POINTS_X; x++) {
                for(uint8_t y = 0; y < GRID_MAX_POINTS_Y; y ++) {
                  ...
                  pos.x = x;
                  pos.y = y;
                  float currval = getMeshPoint(pos);
                  setMeshPoint(pos,constrain(currval + Zshift,AC_LOWEST_MESHPOINT_VAL,2));
                }
              }
              float currZOffset = getZOffset_mm();
              #if ACDEBUG(1)  
                _SELP_4_P(PSTR("Change probe offset from "), currZOffset, PSTR(" to  ") , currZOffset+Zshift );
              #endif
              ...
              if(isAxisPositionKnown(Z)) {
                ...
                float currZpos = getAxisPosition_mm(Z);
                #if ACDEBUG(1) 
                  _SELP_4_P(PSTR("Move Z pos from "), currZpos ,PSTR(" to "), currZpos+constrain(Zshift,-0.05,0.05) );
                #endif
      ...
      case 34: { //A34 Adjust single mesh point A34 C/S X1 Y1 V123
        if(panel_command[3] == 'C') { // Restore original offsets
          ...
          #if ACDEBUG(1) 
            _SELP_4_P(PSTR("Change mesh point x:"),pos.x,PSTR(" y:"),pos.y);
            _SELP_4_P(PSTR("from "),currmesh,PSTR(" to "),newval);
          #endif
          // Update Meshpoint
          ...
          if( (printer_state == AC_printer_idle) || (printer_state == AC_printer_probing) ) {//!isPrinting()) {
            ...
            if((selectedmeshpoint.x == pos.x) && (selectedmeshpoint.y == pos.y)) {
              ...
              float currZpos = getAxisPosition_mm(Z);
              #if ACDEBUG(1) 
                _SELP_4_P(PSTR("Move Z pos from "), currZpos ,PSTR(" to "), currZpos+constrain(newval-currmesh,-0.05,0.05) );
              #endif
