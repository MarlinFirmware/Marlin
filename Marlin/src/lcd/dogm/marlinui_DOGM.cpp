...
void MarlinUI::clear_for_drawing() {
  ...
  void MenuItem_confirm::draw_select_screen(FSTR_P const yes, FSTR_P const no, const bool yesno, FSTR_P const fpre, const char * const string/*=nullptr*/, FSTR_P const fsuf/*=nullptr*/) {
    ...
    void MarlinUI::ubl_plot(const uint8_t x_plot, const uint8_t y_plot) {
      ...
      u8g_uint_t x_map_pixels = ((MAP_MAX_PIXELS_X - 4) / (TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS_X, GRID_MAX_POINTS_X))) * (TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS_X, GRID_MAX_POINTS_X));
      u8g_uint_t y_map_pixels = ((MAP_MAX_PIXELS_Y - 4) / (TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS_Y, GRID_MAX_POINTS_Y))) * (TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS_Y, GRID_MAX_POINTS_Y));

      u8g_uint_t pixels_per_x_mesh_pnt = x_map_pixels / (TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS_X, GRID_MAX_POINTS_X));
      u8g_uint_t pixels_per_y_mesh_pnt = y_map_pixels / (TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS_Y, TERN(VARIABLE_GRID_POINTS, GRID_USED_POINTS_Y, GRID_MAX_POINTS_Y)));

      u8g_uint_t x_offset = MAP_UPPER_LEFT_CORNER_X + 1 + (MAP_MAX_PIXELS_X - x_map_pixels - 2) / 2,
