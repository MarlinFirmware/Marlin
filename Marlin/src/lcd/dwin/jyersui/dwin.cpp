...
private:
  ...
  struct {
    ...
    float getMaxValue() {
      ...
      GRID_LOOP_USED(x, y) { const float z = bedlevel.z_values[x][y]; if (!isnan(z)) NOLESS(max, z); }
      return max;
    }

    float getMinValue() {
      float min = __FLT_MAX__;
      GRID_LOOP_USED(x, y) { const float z = bedlevel.z_values[x][y]; if (!isnan(z)) NOMORE(min, z); }
      return min;
    }
    ...
    void drawBedMesh(const int16_t selected=-1, const uint8_t gridline_width=1, const uint16_t padding_x=8, const uint16_t padding_y_top=40 + 53 - 7) {
      ...
      const uint16_t total_width_px = DWIN_WIDTH - padding_x - padding_x,
                     cell_width_px  = total_width_px / bedlevel.nr_grid_points.x,
                     cell_height_px = total_width_px / bedlevel.nr_grid_points.y;
      ...
      if (selected >= 0) {
        const auto selected_y = selected / bedlevel.nr_grid_points.x,
                   selected_x = selected - bedlevel.nr_grid_points.x * selected_y,
      ...
      GRID_LOOP_USED(x, y) {
        const auto start_x_px = padding_x + x * cell_width_px,
                   ...
                   start_y_px = padding_y_top + (bedlevel.nr_grid_points.y - y - 1) * cell_height_px,
        ...
        if (viewer_print_value) {
          ...
          if (isnan(bedlevel.z_values[x][y])) {  // undefined
            ...
            MString<12> msg;
            if (bedlevel.nr_grid_points.x < 10)
    ...
    case ID_Info:
        ...
        switch (item) {
          ...
          case LEVELING_M_X:
            if (draw) {
              ...
              modifyValue(mesh_conf.mesh_x, 0, bedlevel.nr_grid_points.x - 1, 1);
            break;
          case LEVELING_M_Y:
            if (draw) {
              ...
              modifyValue(mesh_conf.mesh_y, 0, bedlevel.nr_grid_points.y - 1, 1);
            break;
          case LEVELING_M_NEXT:
            if (draw)
              drawMenuItem(row, ICON_More, GET_TEXT_F(MSG_LEVEL_BED_NEXT_POINT));
            else {
              if (mesh_conf.mesh_x != bedlevel.nr_grid_points.x - 1 || mesh_conf.mesh_y != bedlevel.nr_grid_points.y - 1) {
                if ((mesh_conf.mesh_x == bedlevel.nr_grid_points.x - 1 && mesh_conf.mesh_y % 2 == 0) || (mesh_conf.mesh_x == 0 && mesh_conf.mesh_y % 2 == 1))
                  mesh_conf.mesh_y++;
          ...
          case UBL_M_PREV:
            if (draw)
              ...
              if (mesh_conf.mesh_x != 0 || mesh_conf.mesh_y != 0) {
                if ((mesh_conf.mesh_x == bedlevel.nr_grid_points.x - 1 && mesh_conf.mesh_y % 2 == 1) || (mesh_conf.mesh_x == 0 && mesh_conf.mesh_y % 2 == 0))
                  mesh_conf.mesh_y--;
                else if (mesh_conf.mesh_y % 2 == 0)
                  mesh_conf.mesh_x--;
                else
      ...
      case ID_ManualMesh:
        ...
        switch (item) {
          ...
          case MMESH_NEXT:
            if (draw) {
              if (gridpoint < bedlevel.used_points())
                drawMenuItem(row, ICON_More, GET_TEXT_F(MSG_LEVEL_BED_NEXT_POINT));
              else
                drawMenuItem(row, ICON_More, GET_TEXT_F(MSG_UBL_SAVE_MESH));
            }
            else if (gridpoint < bedlevel.used_points()) {
              popupHandler(Popup_MoveWait);
              queue.inject(F("G29"));
              planner.synchronize();
              gridpoint++;
          ...
          case MMESH_OLD:
            ...
            mesh_y = (gridpoint - 1) / bedlevel.nr_grid_points.y;
            mesh_x = (gridpoint - 1) % bedlevel.nr_grid_points.x;

            if (mesh_y % 2 == 1)
              mesh_x = bedlevel.nr_grid_points.x - mesh_x - 1;
