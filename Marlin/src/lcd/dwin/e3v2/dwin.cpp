...
void Item_Prepare_Lang(const uint8_t row) {
  ...
  #endif
  DWIN_Draw_String(false, false, font8x16, HMI_data.Text_Color, HMI_data.Background_Color, 226, MBASE(row), F("EN"));
...
void Draw_Print_ProgressElapsed() {
  ...
  DWIN_Draw_String(false, false, font8x16, HMI_data.Text_Color, HMI_data.Background_Color, 63, 192, F(":"));
...
void Draw_Print_ProgressRemain() {
  ...
  DWIN_Draw_String(false, false, font8x16, HMI_data.Text_Color, HMI_data.Background_Color, 197, 192, F(":"));
...
void _update_axis_value(const AxisEnum axis, const uint16_t x, const uint16_t y, const bool blink, const bool force) {
  ...
  if (force || changed || draw_qmark || draw_empty) {
    if (blink && draw_qmark)
      ...
      DWIN_Draw_Signed_Float(font8x16, HMI_data.Text_Color, HMI_data.Background_Color, 3, 1, x, y, p * 10);
  }
}
...
void update_variable() {
  ...
  if (BABY_Z_VAR != _offset) {
    _offset = BABY_Z_VAR;
    DWIN_Draw_Signed_Float(DWIN_FONT_STAT, HMI_data.Text_Color,  HMI_data.Background_Color, 2, 2, 210, 417, _offset * 100);
  }
...
void Draw_Info_Menu() {
  ...
  DWIN_Draw_String(false, false, font8x16, HMI_data.Text_Color, HMI_data.Background_Color, (DWIN_WIDTH - strlen("Build Datetime") * MENU_CHR_W) / 2, 248, (char*)("Build Datetime"));    // M.A.R.C. Display Firmware build date-time
  DWIN_Draw_String(false, false, font8x16, HMI_data.Text_Color, HMI_data.Background_Color, (DWIN_WIDTH - strlen(STRING_DISTRIBUTION_DATE " " STRING_DISTRIBUTION_TIME) * MENU_CHR_W) / 2, 268, (char*)(STRING_DISTRIBUTION_DATE " " STRING_DISTRIBUTION_TIME));
...
void DWIN_PrinterKilled(PGM_P lcd_error, PGM_P lcd_component) {
  DWIN_Popup_Window(ICON_BLTouch,lcd_error, lcd_component);
  DWIN_UpdateLCD();
}
...
void DWIN_PauseShow(const PauseMessage message) {
  switch (message) {
    ...
    case PAUSE_MESSAGE_UNLOAD:   DWIN_StatusChanged(GET_TEXT(MSG_FILAMENT_CHANGE_UNLOAD)); break;
    case PAUSE_MESSAGE_WAITING:  DWIN_StatusChanged(GET_TEXT(MSG_ADVANCED_PAUSE_WAITING)); break;
    case PAUSE_MESSAGE_INSERT:   DWIN_StatusChanged(GET_TEXT(MSG_FILAMENT_CHANGE_INSERT)); break;
    case PAUSE_MESSAGE_LOAD:     DWIN_StatusChanged(GET_TEXT(MSG_FILAMENT_CHANGE_LOAD));   break;
    case PAUSE_MESSAGE_PURGE:    DWIN_StatusChanged(GET_TEXT(MSG_FILAMENT_CHANGE_PURGE));  break;
