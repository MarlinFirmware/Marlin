  ...
  void menu_advanced_acceleration() {
    ...
    void menu_advanced_input_shaping() {
      ...
      // M593 F Frequency and D Damping ratio
        ...
        if (editable.decimal) {
          ACTION_ITEM_N(X_AXIS, MSG_SHAPING_DISABLE, []{ stepper.set_shaping_frequency(X_AXIS, 0.0f); TERN_(TFT_COLOR_UI, ui.refresh(LCDVIEW_CLEAR_CALL_REDRAW)); });
          ...
          ACTION_ITEM_N(X_AXIS, MSG_SHAPING_ENABLE, []{ stepper.set_shaping_frequency(X_AXIS, (SHAPING_FREQ_X) ?: (SHAPING_MIN_FREQ)); TERN_(TFT_COLOR_UI, ui.refresh(LCDVIEW_CLEAR_CALL_REDRAW)); });
      #endif
      #if ENABLED(INPUT_SHAPING_Y)
        editable.decimal = stepper.get_shaping_frequency(Y_AXIS);
        if (editable.decimal) {
          ACTION_ITEM_N(Y_AXIS, MSG_SHAPING_DISABLE, []{ stepper.set_shaping_frequency(Y_AXIS, 0.0f); TERN_(TFT_COLOR_UI, ui.refresh(LCDVIEW_CLEAR_CALL_REDRAW)); });
          ...
          ACTION_ITEM_N(Y_AXIS, MSG_SHAPING_ENABLE, []{ stepper.set_shaping_frequency(Y_AXIS, (SHAPING_FREQ_Y) ?: (SHAPING_MIN_FREQ)); TERN_(TFT_COLOR_UI, ui.refresh(LCDVIEW_CLEAR_CALL_REDRAW)); });
      #endif
