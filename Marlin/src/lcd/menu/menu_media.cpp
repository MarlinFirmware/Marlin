...
void menu_media_filelist() {
  ...
  void menu_media_select() {
    ...
    BACK_ITEM_F(TERN1(BROWSE_MEDIA_ON_INSERT, screen_history_depth) ? GET_TEXT_F(MSG_MAIN_MENU) : GET_TEXT_F(MSG_BACK));
        ...
        ACTION_ITEM(TERN(SPI_SD_IS_EXTERNAL, MSG_SD_CARD_EXTERNAL, MSG_SD_CARD), []{ card.changeMedia(&card.media_driver_sdcard); card.mount(); ui.goto_screen(menu_media_filelist); });
      #endif
