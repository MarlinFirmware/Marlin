...
void _lcd_level_bed_done()
{
  ...
  if (ui.should_draw())
    MenuItem_static::draw(LCD_HEIGHT >= 4, GET_TEXT(MSG_LEVEL_BED_DONE));
...
void _lcd_level_bed_moving()
{
  ...
  if (!ui.wait_for_move)
    ui.goto_screen(_lcd_level_bed_get_z);
}
...
// Z Fade Height
  ...
  EDIT_ITEM_FAST(float3, MSG_Z_FADE_HEIGHT, &editable.decimal, 0, 100, []
                 { set_z_fade_height(editable.decimal); });
#endif
