...
void _man_probe_pt(const xy_pos_t &xy) {
  ...
  float lcd_probe_pt(const xy_pos_t &xy) {
    ...
    IF_ENABLED(HOST_PROMPT_SUPPORT, hostui.continue_prompt(GET_TEXT_F(MSG_DELTA_CALIBRATION_IN_PROGRESS)));
    IF_ENABLED(EXTENSIBLE_UI, ExtUI::onUserConfirmRequired(GET_TEXT_F(MSG_DELTA_CALIBRATION_IN_PROGRESS)));
  ...
  void _goto_tower_a(const_float_t a) {
    ...
    IF_ENABLED(HAS_PROBE_XY_OFFSET, dcr -= HYPOT(probe.offset_xy.x, probe.offset_xy.y));
...
void lcd_delta_settings() {
  auto _recalc_delta_settings = []{
    IF_ENABLED(HAS_LEVELING, reset_bed_level()); // After changing kinematics bed-level data is no longer valid
