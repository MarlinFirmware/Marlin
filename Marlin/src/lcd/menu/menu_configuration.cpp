...
void menu_advanced_settings();
  ...
  void menu_config_retract() {
    ...
    #endif
    EDIT_ITEM(float3, MSG_CONTROL_RETRACTF, &fwretract.settings.retract_feedrate, 1, 999);
    EDIT_ITEM(float52sign, MSG_CONTROL_RETRACT_ZHOP, &fwretract.settings.retract_zraise, 0, 999);
    EDIT_ITEM(float52sign, MSG_CONTROL_RECOVER, &fwretract.settings.recover_extra, -100, 100);
    #if HAS_MULTI_EXTRUDER
      EDIT_ITEM(float52sign, MSG_CONTROL_RECOVER_SWAP, &fwretract.settings.swap_recover_extra, -100, 100);
    #endif
    EDIT_ITEM(float3, MSG_CONTROL_RECOVERF, &fwretract.settings.recover_feedrate, 1, 999);
    #if HAS_MULTI_EXTRUDER
      EDIT_ITEM(float3, MSG_CONTROL_RECOVER_SWAPF, &fwretract.settings.swap_recover_feedrate, 1, 999);
    #endif
    END_MENU();
  }
