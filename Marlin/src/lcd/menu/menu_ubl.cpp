...
inline int32_t grid_index(const uint8_t x, const uint8_t y) {
  return bedlevel.nr_grid_points.x * y + x;
}
...
void ubl_map_screen() {
  ...
  if (ui.first_page) {
    ...
    do {
      ...
      if (int32_t(ui.encoderPosition) > GRID_MAX_POINTS - 1) ui.encoderPosition = TERN0(TOUCH_SCREEN, ui.encoderPosition - bedlevel.used_points());

      // Draw the grid point based on the encoder
      x = ui.encoderPosition % bedlevel.nr_grid_points.x;
      y = ui.encoderPosition / bedlevel.nr_grid_points.x;
    ...
    #else
      const bool xc = WITHIN(x, 1, bedlevel.nr_grid_points.x - 2),
                 yc = WITHIN(y, 1, bedlevel.nr_grid_points.y - 2);
