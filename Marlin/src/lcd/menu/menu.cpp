    ...
    if (ui.encoderPosition) {
      ...
      const float diff = planner.mm_per_step[Z_AXIS] * babystep_increment,
                  ...
                  new_offs = TERN(BABYSTEP_HOTEND_Z_OFFSET
                    , do_probe ? new_probe_offset : motion.active_tool_offset().z - diff
                    , new_probe_offset
                  );
      if (WITHIN(new_offs, PROBE_OFFSET_ZMIN, PROBE_OFFSET_ZMAX)) {
        ...
        if (do_probe)
          ...
          TERN(BABYSTEP_HOTEND_Z_OFFSET, motion.active_tool_offset().z = new_offs, NOOP);
    ...
    if (ui.should_draw()) {
      if (do_probe) {
        ...
        TERN_(BABYSTEP_GFX_OVERLAY, ui.zoffset_overlay(probe.offset.z));
          ...
          MenuEditItemBase::draw_edit_screen(GET_TEXT_F(MSG_TOOL_OFFSET_Z), ftostr54sign(motion.active_tool_offset().z));
        #endif
      }
    }
  }
