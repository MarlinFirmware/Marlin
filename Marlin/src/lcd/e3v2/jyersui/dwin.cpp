    ...
    GRID_LOOP(x, y)
    {
      ...
      DWIN_Draw_Rectangle(1,                                                                                                                                                                                     // RGB565 colors: http://www.barth-dev.de/online/rgb565-color-picker/
                          isnan(bedlevel.z_values[x][y]) ? Color_Grey : (                                                                                                                                        // gray if undefined
                                                                            (bedlevel.z_values[x][y] < 0 ? (uint16_t)round(0x1F * -bedlevel.z_values[x][y] / (!viewer_asymmetric_range ? range : v_min)) << 11 : // red if mesh point value is negative
...
struct CrealityDWINClass::EEPROM_Settings CrealityDWINClass::eeprom_settings
{
  0
};
...
{
  if (e == 1 || e == 3 || e == 4)
    DWIN_Draw_Rectangle(1, GetColor(eeprom_settings.menu_top_bg, Color_Bg_Blue, false), 0, 0, DWIN_WIDTH, TITLE_HEIGHT); // Clear Title Bar
  if (e == 2 || e == 3)
    DWIN_Draw_Rectangle(1, Color_Bg_Black, 0, 31, DWIN_WIDTH, STATUS_Y); // Clear Menu Area
  if (e == 4)
    DWIN_Draw_Rectangle(1, Color_Bg_Black, 0, 31, DWIN_WIDTH, DWIN_HEIGHT); // Clear Popup Area
}
...
{
  if (icon)
    DWIN_ICON_Show(ICON, icon, 26, MBASE(row) - 3); //Draw Menu Icon
  if (more)
    DWIN_ICON_Show(ICON, ICON_More, 226, MBASE(row) - 3);                                                                                           // Draw More Arrow
...
void CrealityDWINClass::Draw_Menu_Item(uint8_t row, uint8_t icon /*=0*/, const char *label1, const char *label2, bool more /*=false*/, bool centered /*=false*/)
{
  ...
  if (label1)
    DWIN_Draw_String(false, DWIN_FONT_MENU, Color_White, Color_Bg_Black, label1_offset_x, MBASE(row) - 1 - label_offset_y, label1); // Draw Label
  if (label2)
    DWIN_Draw_String(false, DWIN_FONT_MENU, Color_White, Color_Bg_Black, label2_offset_x, MBASE(row) - 1 + label_offset_y, label2); // Draw Label
...
void CrealityDWINClass::Draw_Menu_Item(uint8_t row, uint8_t icon /*=0*/, FSTR_P const flabel1, FSTR_P const flabel2, bool more /*=false*/, bool centered /*=false*/)
{
  ...
  if (flabel1)
    DWIN_Draw_String(false, DWIN_FONT_MENU, Color_White, Color_Bg_Black, label1_offset_x, MBASE(row) - 1 - label_offset_y, flabel1); // Draw Label
  if (flabel2)
    DWIN_Draw_String(false, DWIN_FONT_MENU, Color_White, Color_Bg_Black, label2_offset_x, MBASE(row) - 1 + label_offset_y, flabel2); // Draw Label
  _Decorate_Menu_Item(row, icon, more);
}
...
void CrealityDWINClass::Draw_Menu(uint8_t menu, uint8_t select /*=0*/, uint8_t scroll /*=0*/)
{
  ...
  LOOP_L_N(i, TROWS)
  Menu_Item_Handler(menu, i + scrollpos);
...
{
  if (process != Confirm && process != Popup && process != Wait)
    last_process = process;
  if ((process == Menu || process == Wait) && mode == Popup)
    last_selection = selection;
  process = mode;
  Clear_Screen();
...
#define PREHEAT_TOTAL PREHEAT_5
    ...
    auto do_preheat = [](const uint8_t m)
    {
      ...
      {
        ui.preheat_hotend_and_fan(m);
      }
      if (preheatmode == 0 || preheatmode == 2)
        ui.preheat_bed(m);
    };
...
#define _PREHEAT_CASE(N)                                             \
  ...
  {                                                                  \
    if (draw)                                                        \
      Draw_Menu_Item(row, ICON_Temperature, F(PREHEAT_##N##_LABEL)); \
    else                                                             \
      do_preheat(N - 1);                                             \
  }                                                                  \
  break;
...
#define _TEMP_PREHEAT_CASE(N)                                                \
  ...
  {                                                                          \
    if (draw)                                                                \
      Draw_Menu_Item(row, ICON_Step, F(PREHEAT_##N##_LABEL), nullptr, true); \
    else                                                                     \
      Draw_Menu(Preheat##N);                                                 \
  }                                                                          \
  break;
...
#if HAS_PREHEAT
#define _PREHEAT_SUBMENU_CASE(N)                   \
  case Preheat##N:                                 \
    preheat_submenu((N)-1, item, TEMP_PREHEAT##N); \
    break;
...
#define TMC_TOTAL TMC_STEPPER_CURRENT_E
    ...
    case TMC_STEPPER_CURRENT_X:
      ...
      {
        Modify_Value(stepper_current_x, TMC_MIN_CURRENT, TMC_MAX_CURRENT, 1, []
                     { stepperX.rms_current(stepper_current_x); });
      }
      break;
#endif
    ...
    case TMC_STEPPER_CURRENT_Y:
      ...
      {
        Modify_Value(stepper_current_y, TMC_MIN_CURRENT, TMC_MAX_CURRENT, 1, []
                     { stepperY.rms_current(stepper_current_y); });
      }
      break;
#endif
    ...
    case TMC_STEPPER_CURRENT_Z:
      ...
      {
        Modify_Value(stepper_current_z, TMC_MIN_CURRENT, TMC_MAX_CURRENT, 1, []
                     { stepperZ.rms_current(stepper_current_z); });
      }
      break;
#endif
    ...
    case TMC_STEPPER_CURRENT_E:
      ...
      {
        Modify_Value(stepper_current_e, TMC_MIN_CURRENT, TMC_MAX_CURRENT, 1, []
                     { stepperE0.rms_current(stepper_current_e); });
      }
      break;
#endif
    };
...
FSTR_P CrealityDWINClass::Get_Menu_Title(uint8_t menu)
{
  ...
  case Prepare:
    return F("Prepare");
  case HomeMenu:
    return F("Homing Menu");
  case Move:
    return F("Move");
  case ManualLevel:
    return F("Manual Leveling");
#if HAS_ZOFFSET_ITEM
  case ZOffset:
    return F("Z Offset");
#endif
...
#if HAS_HOTEND
  case HotendPID:
    return F("Hotend PID Settings");
#endif
#if HAS_HEATED_BED
  case BedPID:
    return F("Bed PID Settings");
#endif
#if HAS_PREHEAT
#define _PREHEAT_TITLE_CASE(N) \
  case Preheat##N:             \
    return F(PREHEAT_##N##_LABEL " Settings");
    REPEAT_1(PREHEAT_COUNT, _PREHEAT_TITLE_CASE)
#endif
  case Motion:
    return F("Motion Settings");
  case HomeOffsets:
    return F("Home Offsets");
  case MaxSpeed:
    return F("Max Speed");
  case MaxAcceleration:
    return F("Max Acceleration");
#if HAS_CLASSIC_JERK
  case MaxJerk:
    return F("Max Jerk");
#endif
  case Steps:
    return F("Steps/mm");
  case Visual:
    return F("Visual Settings");
  case Advanced:
    return F("Advanced Settings");
#if HAS_BED_PROBE
  case ProbeMenu:
    return F("Bed Probe");
#endif
#if HAS_TRINAMIC_CONFIG
  case TMCMenu:
    return F("TMC Drivers");
#endif
  case ColorSettings:
    return F("UI Color Settings");
  case Info:
    return F("Info");
  case InfoMain:
    return F("Info");
  ...
  case Leveling:
    return F("Leveling");
  case LevelView:
    return GET_TEXT_F(MSG_MESH_VIEW);
  case LevelSettings:
    return F("Leveling Settings");
  case MeshViewer:
    return GET_TEXT_F(MSG_MESH_VIEW);
  case LevelManual:
    return F("Manual Tuning");
#endif
#if ENABLED(AUTO_BED_LEVELING_UBL) && !HAS_BED_PROBE
  case UBLMesh:
    return F("UBL Bed Leveling");
#endif
#if ENABLED(PROBE_MANUALLY)
  case ManualMesh:
    return F("Mesh Bed Leveling");
#endif
  case Tune:
    return F("Tune");
  case PreheatHotend:
    return F("Preheat Hotend");
  }
  return F("");
}
...
void CrealityDWINClass::Popup_Handler(PopupID popupid, bool option /*=false*/)
{
  ...
  {
  case Pause:
    Draw_Popup(F("Pause Print"), F(""), F(""), Popup);
    break;
  case Stop:
    Draw_Popup(F("Stop Print"), F(""), F(""), Popup);
    break;
  case Resume:
    Draw_Popup(F("Resume Print?"), F("Looks Like the last"), F("print was interrupted."), Popup);
    break;
  case ConfFilChange:
    Draw_Popup(F("Confirm Filament Change"), F(""), F(""), Popup);
    break;
  case PurgeMore:
    Draw_Popup(F("Purge more filament?"), F("(Cancel to finish process)"), F(""), Popup);
    break;
  case SaveLevel:
    Draw_Popup(F("Leveling Complete"), F("Save to EEPROM?"), F(""), Popup);
    break;
  case MeshSlot:
    Draw_Popup(F("Mesh slot not selected"), F("(Confirm to select slot 0)"), F(""), Popup);
    break;
  case ETemp:
    Draw_Popup(F("Nozzle is too cold"), F("Open Preheat Menu?"), F(""), Popup);
    break;
  case ManualProbing:
    Draw_Popup(F("Manual Probing"), F("(Confirm to probe)"), F("(cancel to exit)"), Popup);
    break;
  case Level:
    Draw_Popup(F("Auto Bed Leveling"), F("Please wait until done."), F(""), Wait, ICON_AutoLeveling);
    break;
  case Home:
    Draw_Popup(option ? F("Parking") : F("Homing"), F("Please wait until done."), F(""), Wait, ICON_BLTouch);
    break;
  case MoveWait:
    Draw_Popup(F("Moving to Point"), F("Please wait until done."), F(""), Wait, ICON_BLTouch);
    break;
  case Heating:
    Draw_Popup(F("Heating"), F("Please wait until done."), F(""), Wait, ICON_BLTouch);
    break;
  case FilLoad:
    Draw_Popup(option ? F("Unloading Filament") : F("Loading Filament"), F("Please wait until done."), F(""), Wait, ICON_BLTouch);
    break;
  case FilChange:
    Draw_Popup(F("Filament Change"), F("Please wait for prompt."), F(""), Wait, ICON_BLTouch);
    break;
  case TempWarn:
    Draw_Popup(option ? F("Nozzle temp too low!") : F("Nozzle temp too high!"), F(""), F(""), Wait, option ? ICON_TempTooLow : ICON_TempTooHigh);
    break;
  case Runout:
    Draw_Popup(F("Filament Runout"), F(""), F(""), Wait, ICON_BLTouch);
    break;
  case PIDWait:
    Draw_Popup(F("PID Autotune"), F("in process"), F("Please wait until done."), Wait, ICON_BLTouch);
    break;
  case Resuming:
    Draw_Popup(F("Resuming Print"), F("Please wait until done."), F(""), Wait, ICON_BLTouch);
    break;
  case Custom:
    Draw_Popup(F("Running Custom GCode"), F("Please wait until done."), F(""), Wait, ICON_BLTouch);
    break;
  default:
    break;
  }
...
void CrealityDWINClass::Confirm_Handler(PopupID popupid)
{
  ...
  {
  case FilInsert:
    Draw_Popup(F("Insert Filament"), F("Press to Continue"), F(""), Confirm);
    break;
  case HeaterTime:
    Draw_Popup(F("Heater Timed Out"), F("Press to Reheat"), F(""), Confirm);
    break;
  case UserInput:
    Draw_Popup(F("Waiting for Input"), F("Press to Continue"), F(""), Confirm);
    break;
  case LevelError:
    Draw_Popup(F("Couldn't enable Leveling"), F("(Valid mesh must exist)"), F(""), Confirm);
    break;
  case InvalidMesh:
    Draw_Popup(F("Valid mesh must exist"), F("before tuning can be"), F("performed"), Confirm);
    break;
  default:
    break;
  }
...
void CrealityDWINClass::Main_Menu_Control()
{
  ...
  else if (encoder_diffState == ENCODER_DIFF_CCW && selection > 0)
  {
    ...
    case PAGE_INFO_LEVELING:
      Draw_Menu(TERN(HAS_MESH, Leveling, InfoMain));
      break;
    }
  DWIN_UpdateLCD();
}
  ...
  else if (encoder_diffState == ENCODER_DIFF_ENTER)
  {
    ...
    case 1:
      *(uint8_t *)valuepointer = tempvalue / valueunit;
      break;
    case 2:
      *(uint16_t *)valuepointer = tempvalue / valueunit;
      break;
    case 3:
      *(int16_t *)valuepointer = tempvalue / valueunit;
      break;
    case 4:
      *(uint32_t *)valuepointer = tempvalue / valueunit;
      break;
    case 5:
      *(int8_t *)valuepointer = tempvalue / valueunit;
      break;
    }
    switch (active_menu)
    {
    ...
    case UBLMesh:
      ...
      break;
    case LevelManual:
      mesh_conf.manual_mesh_move(selection == LEVELING_M_OFFSET);
      break;
#endif
    }
    if (funcpointer)
...
void CrealityDWINClass::Option_Control()
{
  ...
  else if (encoder_diffState == ENCODER_DIFF_ENTER)
  {
    ...
    if (valuepointer == &color_names)
    {
      ...
      {
      case COLORSETTINGS_CURSOR:
        eeprom_settings.cursor_color = tempvalue;
        break;
      case COLORSETTINGS_SPLIT_LINE:
        eeprom_settings.menu_split_line = tempvalue;
        break;
      case COLORSETTINGS_MENU_TOP_BG:
        eeprom_settings.menu_top_bg = tempvalue;
        break;
      case COLORSETTINGS_MENU_TOP_TXT:
        eeprom_settings.menu_top_txt = tempvalue;
        break;
      case COLORSETTINGS_HIGHLIGHT_BORDER:
        eeprom_settings.highlight_box = tempvalue;
        break;
      case COLORSETTINGS_PROGRESS_PERCENT:
        eeprom_settings.progress_percent = tempvalue;
        break;
      case COLORSETTINGS_PROGRESS_TIME:
        eeprom_settings.progress_time = tempvalue;
        break;
      case COLORSETTINGS_PROGRESS_STATUS_BAR:
        eeprom_settings.status_bar_text = tempvalue;
        break;
      case COLORSETTINGS_PROGRESS_STATUS_AREA:
        eeprom_settings.status_area_text = tempvalue;
        break;
      case COLORSETTINGS_PROGRESS_COORDINATES:
        eeprom_settings.coordinates_text = tempvalue;
        break;
      case COLORSETTINGS_PROGRESS_COORDINATES_LINE:
        eeprom_settings.coordinates_split_line = tempvalue;
        break;
      }
      Redraw_Screen();
    }
...
void CrealityDWINClass::Update_Status(const char *const text)
{
  ...
  {
    LOOP_L_N(i, _MIN((size_t)LONG_FILENAME_LENGTH, strlen(text)))
    filename[i] = text[i + 3];
  ...
  {
    LOOP_L_N(i, _MIN((size_t)64, strlen(text)))
    statusmsg[i] = text[i];
...
void CrealityDWINClass::State_Update()
{
  ...
  {
    if (!printing)
      Start_Print(card.isFileOpen() || TERN0(POWER_LOSS_RECOVERY, recovery.valid()));
    else
      Stop_Print();
  }
...
void CrealityDWINClass::Load_Settings(const char *buff)
{
  ...
  if (eeprom_settings.corner_pos == 0)
