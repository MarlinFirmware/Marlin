...
EncoderState Encoder_ReceiveAnalyze()
{
  ...
  if (newbutton != lastEncoderBits)
  {
    ...
    {
    case 0:
      if (lastEncoderBits == 1)
        temp_diff++;
      else if (lastEncoderBits == 2)
        temp_diff--;
      break;
    case 2:
      if (lastEncoderBits == 0)
        temp_diff++;
      else if (lastEncoderBits == 3)
        temp_diff--;
      break;
    case 3:
      if (lastEncoderBits == 2)
        temp_diff++;
      else if (lastEncoderBits == 1)
        temp_diff--;
      break;
    case 1:
      if (lastEncoderBits == 3)
        temp_diff++;
      else if (lastEncoderBits == 0)
        temp_diff--;
      break;
    }
  ...
  {
    if (temp_diff > 0)
      temp_diffState = TERN(REVERSE_ENCODER_DIRECTION, ENCODER_DIFF_CCW, ENCODER_DIFF_CW);
    else
      temp_diffState = TERN(REVERSE_ENCODER_DIRECTION, ENCODER_DIFF_CW, ENCODER_DIFF_CCW);
    ...
    if (EncoderRate.enabled)
    {
      ...
      if (EncoderRate.lastEncoderTime)
      {
        ...
        else if (encoderStepRate >= ENCODER_5X_STEPS_PER_SEC)
          encoderMultiplier = 5;
#endif
      }
