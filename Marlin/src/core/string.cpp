...
const ROString ROString::fromTo(const ROString & from, const ROString & to, const bool includeFind) const {
...
const ROString ROString::upToFirst(const ROString & f, const bool includeFind) const {
  const unsigned int pos = find(f);
...
const ROString ROString::upToLast(const ROString & f, const bool includeFind) const {
...
// Get the string from the first occurrence of the given string
const ROString ROString::dropUpTo(const ROString & f, const bool includeFind) const {
  const unsigned int pos = find(f);
...
const ROString ROString::splitUpTo(const ROString & f, const bool includeFind) {
  const unsigned int pos = find(f);
  if (pos == (unsigned int)length) {
    ROString ret(*this);
    (void)mutate(data + length, 0);
