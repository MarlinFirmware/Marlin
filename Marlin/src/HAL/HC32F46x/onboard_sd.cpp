  ...
  {                            /* Put the card SPI state */
    ...
    { /* Is the catd SDv2? */
      ...
      { /* Does the card support 2.7-3.6V? */
        while ((timeout > millis()) && send_cmd(ACMD41, 1UL << 30))
          ; /* Wait for end of initialization with ACMD41(HCS) */
        if ((timeout > millis()) && send_cmd(CMD58, 0) == 0)
    ...
    { /* Not an SDv2 card */
      ...
      while ((timeout > millis()) && send_cmd(cmd, 0))
...
)
{
  ...
  case ISDIO_READ:
    ...
    if (send_cmd(CMD48, 0x80000000 | sdio->func << 28 | sdio->addr << 9 | ((sdio->ndata - 1) & 0x1FF)) == 0)
    {
      ...
      if (rc == 0xFE)
      {
        for (buf = sdio->data, dc = sdio->ndata; dc; dc--)
          *buf++ = xchg_spi(0xFF);
        for (dc = 514 - sdio->ndata; dc; dc--)
          xchg_spi(0xFF);
        res = RES_OK;
      }
    }
