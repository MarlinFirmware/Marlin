...
#define SERVV_MIN(v) (MIN_PULSE_WIDTH - (v) * 4) // minimum value in uS for this servo
#define SERVV_MAX(v) (MAX_PULSE_WIDTH - (v) * 4) // maximum value in uS for this servo
...
void Servo::write(int value) {
  if (value < MIN_PULSE_WIDTH)    // treat values less than 544 as angles in degrees (valid values in microseconds are handled as microseconds)
    value = map(constrain(value, 0, 180), 0, 180, SERVV_MIN(min), SERVV_MAX(max));
...
void Servo::writeMicroseconds(int value) {
  ...
  if (channel < MAX_SERVOS) {  // ensure channel is valid
    ...
    value = constrain(value, SERVV_MIN(min), SERVV_MAX(max)) - (TRIM_DURATION);
...
int Servo::read() { return map(readMicroseconds() + 1, SERVV_MIN(min), SERVV_MAX(max), 0, 180); }
