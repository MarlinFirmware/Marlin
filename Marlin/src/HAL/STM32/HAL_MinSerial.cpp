...
#if DISABLED(DYNAMIC_VECTORTABLE) && DISABLED(STM32F0xx) && DISABLED(STM32G0xx) // Cortex M0 can't jump to a symbol that's too far from the current function, so we work around this in exception_arm.cpp
extern "C" {
