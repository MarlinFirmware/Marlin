...
uint32_t CAN_next_temp_report_time                 = 12000; // Track when the next toolhead temperature report should arrive, delay at startup
...
HAL_StatusTypeDef CAN_host_send_gcode() { // Forward a Marlin Gcode via CAN (uses parser.command_letter, Gcode_no, parser.value_float())
  ...
  if ((Gcode_no != 104) &&  // Set hotend target temp
      ...
      (Gcode_no != 119) &&  // Endstop status (for testing, not needed)
