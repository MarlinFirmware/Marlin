...
//
// Define the array in PROGMEM. This file should only be included by MarlinCore.cpp.
//
constexpr pin_t sensitive_pins[] PROGMEM = {
  ...
  #if HAS_HOTEND
    HEATER_0_PIN, DIO_PIN(TEMP_0_PIN),
    #if PIN_EXISTS(E0_AUTO_FAN)
      E0_AUTO_FAN_PIN,
    #endif
    #if HAS_MULTI_HOTEND
      HEATER_1_PIN, DIO_PIN(TEMP_1_PIN),
      #if PIN_EXISTS(E1_AUTO_FAN)
        E1_AUTO_FAN_PIN,
      #endif
      #if HOTENDS > 2
        HEATER_2_PIN, DIO_PIN(TEMP_2_PIN),
        #if PIN_EXISTS(E2_AUTO_FAN)
          E2_AUTO_FAN_PIN,
        #endif
        #if HOTENDS > 3
          HEATER_3_PIN, DIO_PIN(TEMP_3_PIN),
          #if PIN_EXISTS(E3_AUTO_FAN)
            E3_AUTO_FAN_PIN,
          #endif
          #if HOTENDS > 4
            HEATER_4_PIN, DIO_PIN(TEMP_4_PIN),
            #if PIN_EXISTS(E4_AUTO_FAN)
              E4_AUTO_FAN_PIN,
            #endif
            #if HOTENDS > 5
              HEATER_5_PIN, DIO_PIN(TEMP_5_PIN),
              #if PIN_EXISTS(E5_AUTO_FAN)
                E5_AUTO_FAN_PIN,
              #endif
              #if HOTENDS > 6
                HEATER_6_PIN, DIO_PIN(TEMP_6_PIN),
                #if PIN_EXISTS(E6_AUTO_FAN)
                  E6_AUTO_FAN_PIN,
                #endif
                #if HOTENDS > 7
                  HEATER_7_PIN, DIO_PIN(TEMP_7_PIN),
  ...
  //
    ...
    HEATER_BED_PIN, DIO_PIN(TEMP_BED_PIN),
  #endif

  #if TEMP_SENSOR_CHAMBER && PIN_EXISTS(TEMP_CHAMBER)
    DIO_PIN(TEMP_CHAMBER_PIN),
