 ...
 */
  ...
  #error "FILAMENT_RUNOUT_SENSOR is now FILAMENT_RUNOUT_MULTI_SENSORS. Please update your configuration."  
#endif   
#if ENABLED(FILAMENT_RUNOUT_MULTI_SENSORS)
  #if FIL_RUNOUT_SENSORS >5 || FIL_RUNOUT_SENSORS <1
    #error "FILAMENT_RUNOUT_MULTI_SENSORS Number of sensors error"
  #elif  FIL_RUNOUT_SENSORS >0 && !HAS_FIL_RUNOUT0
    #error "FILAMENT_RUNOUT_MULTI_SENSORS requires FIL_RUNOUT_0_PIN"
  #elif  FIL_RUNOUT_SENSORS >1 && (!HAS_FIL_RUNOUT0 || !HAS_FIL_RUNOUT1)
    #error "FILAMENT_RUNOUT_MULTI_SENSORS requires FIL_RUNOUT_PIN 0 to 1"
  #elif  FIL_RUNOUT_SENSORS >2 && (!HAS_FIL_RUNOUT0 || !HAS_FIL_RUNOUT1 || !HAS_FIL_RUNOUT2)
    #error "FILAMENT_RUNOUT_MULTI_SENSORS requires FIL_RUNOUT_PIN 0 to 2"
  #elif  FIL_RUNOUT_SENSORS >3 && (!HAS_FIL_RUNOUT0 || !HAS_FIL_RUNOUT1 || !HAS_FIL_RUNOUT2 || !HAS_FIL_RUNOUT3)
    #error "FILAMENT_RUNOUT_MULTI_SENSORS requires FIL_RUNOUT_PIN 0 to 3"
  #elif  FIL_RUNOUT_SENSORS >4 && (!HAS_FIL_RUNOUT0 || !HAS_FIL_RUNOUT1 || !HAS_FIL_RUNOUT2 || !HAS_FIL_RUNOUT3 || !HAS_FIL_RUNOUT4)
    #error "FILAMENT_RUNOUT_MULTI_SENSORS requires FIL_RUNOUT_PIN 0 to 4"
  #elif DISABLED(SDSUPPORT) && DISABLED(PRINTJOB_TIMER_AUTOSTART)
    #error "FILAMENT_RUNOUT_MULTI_SENSORS requires SDSUPPORT or PRINTJOB_TIMER_AUTOSTART."
  #elif DISABLED(SMART_PAUSE)
    static_assert(NULL == strstr(FILAMENT_RUNOUT_SCRIPT, "M600"), "SMART_PAUSE is required to use M600 with FILAMENT_RUNOUT_MULTI_SENSORS.");
  #endif
#endif
 ...
 */
    ...
    #error "SMART_PAUSE currently requires an LCD controller."
  #elif ENABLED(EXTRUDER_RUNOUT_PREVENT)
    #error "EXTRUDER_RUNOUT_PREVENT is incompatible with SMART_PAUSE."
  #elif ENABLED(PARK_HEAD_ON_PAUSE) && DISABLED(SDSUPPORT) && DISABLED(NEWPANEL) && DISABLED(EMERGENCY_PARSER)
    #error "PARK_HEAD_ON_PAUSE requires SDSUPPORT, EMERGENCY_PARSER, or an LCD controller."
  #elif ENABLED(PAUSE_HOME_BEFORE ) && DISABLED(PAUSE_STEPPER_ON )
    #error "PAUSE_HOME_BEFORE  requires PAUSE_STEPPER_ON "
  #endif
  #if ENABLED(PAUSE_SPOOL_SWAP)
    #if EXTRUDERS<2 || FIL_RUNOUT_SENSORS <2
	#error "PAUSE_SPOOL_SWAP requires 2 Extruders and 2 Runout Sensors at min"
    #endif
  #endif
#endif
