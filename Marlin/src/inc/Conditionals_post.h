...
#define _HAS_STOP(A,M) (ENABLED(Z_MIN_PROBE_USES_##A##_##M##_ENDSTOP_PIN) \
  || (HAS_##A##_AXIS && A##HOME_TO_##M && PIN_EXISTS(A##_##M) && !IS_PROBE_PIN(A,M) \
      && !IS_X2_ENDSTOP(A,M) && !IS_Y2_ENDSTOP(A,M) \
      && !IS_Z2_ENDSTOP(A,M) && !IS_Z3_ENDSTOP(A,M) && !IS_Z4_ENDSTOP(A,M)))
#if _HAS_STOP(X,MIN)
  #define HAS_X_MIN 1
#endif
#if _HAS_STOP(X,MAX)
