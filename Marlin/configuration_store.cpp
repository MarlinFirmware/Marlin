...
void MarlinSettings::postprocess() {
  ...
  bool MarlinSettings::load() {
  ...
  #if ENABLED(AUTO_BED_LEVELING_UBL)
    int MarlinSettings::get_start_of_meshes() {
      return meshes_begin;
    }

    int MarlinSettings::get_end_of_meshes() {
      return meshes_end;
    }
    ...
    void MarlinSettings::load_mesh(int8_t slot, void *into /* = 0 */) {
        ...
        if (into)
          read_data(pos, (uint8_t *)into, sizeof(ubl.z_values), &crc);
        else
          read_data(pos, (uint8_t *)&ubl.z_values, sizeof(ubl.z_values), &crc);
