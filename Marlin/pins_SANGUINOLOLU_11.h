...
#if !MB(AZTEEG_X1) && ENABLED(SPINDLE_LASER_ENABLE) && ENABLED(SANGUINOLOLU_V_1_2) && !(ENABLED(ULTRA_LCD) && ENABLED(NEWPANEL))  // try to use IO Header
  #define SPINDLE_LASER_ENABLE_PIN  10     // should have a pull up/down on it
  #define SPINDLE_SPEED_LASER_POWER_PIN    4     // MUST BE A HARDWARE PWM
  #define SPINDLE_DIR_PIN     11
#elif  ENABLED(SPINDLE_LASER_ENABLE) && !MB(MELZI)  // use X stepper motor socket
 ...
 *                          /RESET   O|     |O  1A
 *                          /SLEEP   O|     |O  1B
 *      SPINDLE_SPEED_LASER_POWER_PIN   STEP     O|     |O  VDD
 ...
 */
  ...
  #define X_ENABLE_PIN          14
  #define X_STEP_PIN             1
  #define SPINDLE_SPEED_LASER_POWER_PIN     15
  #define SPINDLE_LASER_ENABLE_PIN    21  // should have a pullup resistor on this pin
  #define SPINDLE_DIR_PIN       -1  // don't have a pin available on the socket for the direction pin
#endif

// misc
#if !ENABLED(SPINDLE_LASER_ENABLE) && ENABLED(SANGUINOLOLU_V_1_2) && !(ENABLED(ULTRA_LCD) && ENABLED(NEWPANEL))  // try to use IO Header
