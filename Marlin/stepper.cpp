...
volatile unsigned long Stepper::step_events_completed = 0; // The number of step events executed in the current block
  ...
  volatile unsigned char Stepper::eISR_Rate = 200; // Keep the ISR at a low rate until needed
    ...
    volatile int Stepper::e_steps[ADVANCE_STEPPERS];
    int Stepper::extruder_advance_k = LIN_ADVANCE_K,
        Stepper::final_estep_rate,
        Stepper::current_estep_rate[ADVANCE_STEPPERS],
        Stepper::current_adv_steps[ADVANCE_STEPPERS];
  #else
    long  Stepper::e_steps[ADVANCE_STEPPERS],
  ...
  if (current_block != NULL) {
    ...
    for (int8_t i = 0; i < step_loops; i++) {
        ...
        if (counter_E > 0) {
          ...
          // Don't advance E here for mixing extruder
          if (current_block->use_advance_lead) {
            int delta_adv_steps = (((long)extruder_advance_k * current_estep_rate[current_block->active_extruder]) >> 9) - current_adv_steps[current_block->active_extruder];
          ...
          // Apply advance lead proportionally (??)
          if (current_block->use_advance_lead) {
            int delta_adv_steps = (((long)extruder_advance_k * current_estep_rate[current_block->active_extruder]) >> 9) - current_adv_steps[current_block->active_extruder];
            MIXING_STEPPERS_LOOP(j) {
    ...
    if (step_events_completed <= (unsigned long)current_block->accelerate_until) {
      ...
      acceleration_time += timer;
        ...
        // Do E steps + advance steps
          ...
          e_steps[current_block->active_extruder] += advance_factor;
        #endif
...
void Stepper::init() {
  ...
  ENABLE_STEPPER_DRIVER_INTERRUPT();
      ...
      for (int i = 0; i < ADVANCE_STEPPERS; i++) {
        e_steps[i] = 0;
      ...
      for (uint8_t i = 0; i < ADVANCE_STEPPERS; i++) e_steps[i] = 0;
