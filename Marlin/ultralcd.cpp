...
void lcd_set_home_offsets() {
  ...
  static void _lcd_babystep(int axis, char *msg) {
    ...
    if (lcdDrawUpdate) {
      lcd_implementation_drawedit(PSTR(msg), "");
    }
    if (LCD_CLICKED) {
      lcd_quick_feedback();
...
static void _lcd_move(char *name, int axis, int min, int max) {
  if (encoderPosition != 0) {
    refresh_cmd_timeout();
...
void lcd_reset_alert_level() { lcd_status_message_level = 0; }
  ...
  void lcd_buttons_update() {
    ...
    uint8_t enc=0;
    if (buttons & EN_A) enc |= (1 << 0);
    if (buttons & EN_B) enc |= (1 << 1);
...
//  convert float to zero-padded string with +123.4 format
...
//  convert float to zero-padded string with 123.4 format, dropping sign
...
//  convert float to zero-padded string with +123.45 format
