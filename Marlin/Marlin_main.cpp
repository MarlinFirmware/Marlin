...
static void clean_up_after_endstop_or_probe_move() {
  ...
  bool set_probe_deployed(bool deploy) {
    ...
    //do_blocking_move_to(oldXpos, oldYpos, current_position[Z_AXIS]); // return to position before deploy  TFs mod - final positions defined in scripts only
  ...
  float probe_pt(const float &lx, const float &ly, const bool stow, const uint8_t verbose_level, const bool printable=true) {
    ...
    const float nx = lx - (X_PROBE_OFFSET_FROM_EXTRUDER), ny = ly - (Y_PROBE_OFFSET_FROM_EXTRUDER); //TFS mod test (2 * Y_PROBE_OFFSET_FROM_EXTRUDER) instead (Y_PROBE_OFFSET_FROM_EXTRUDER)
    ...
    do_blocking_move_to_xy(nx, ny); //TFS mod test (nx - (X_PROBE_OFFSET_FROM_EXTRUDER), ny - (Y_PROBE_OFFSET_FROM_EXTRUDER) instead (nx, ny)
