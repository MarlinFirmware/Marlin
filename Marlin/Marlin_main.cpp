...
void clean_up_after_endstop_or_probe_move() {
  ...
  // TRIGGERED_WHEN_STOWED_TEST can easily be extended to servo probes, ... if needed.
    ...
    bool bltouch_deploy_proc() {
      // Do a DEPLOY
      #if ENABLED(DEBUG_LEVELING_FEATURE)
        if (DEBUGGING(LEVELING)) SERIAL_ECHO("BLTouch DEPLOY requested");
      #endif
      ...
      if (_bltouch_deploy_query_alarm()) {
        ...
        #if ENABLED(DEBUG_LEVELING_FEATURE)
          if (DEBUGGING(LEVELING)) SERIAL_ECHO("BLTouch ALARM or TRIGGER after DEPLOY, recovering");
        #endif
        ...
        if (_bltouch_deploy_query_alarm()) {
          ...
          #if ENABLED(DEBUG_LEVELING_FEATURE)
            if (DEBUGGING(LEVELING)) SERIAL_ECHO("BLTouch Recovery Failed");
          #endif
      ...
      // is registered.
      #if ENABLED(DEBUG_LEVELING_FEATURE)
        if (DEBUGGING(LEVELING)) SERIAL_ECHO("bltouch.deploy_proc() end");
      #endif
    ...
    bool bltouch_stow_proc() {
      // Do a STOW
      #if ENABLED(DEBUG_LEVELING_FEATURE)
        if (DEBUGGING(LEVELING)) SERIAL_ECHO("BLTouch STOW requested");
      #endif
      ...
      if (_bltouch_stow_query_alarm()) {
        ...
        #if ENABLED(DEBUG_LEVELING_FEATURE)
          if (DEBUGGING(LEVELING)) SERIAL_ECHO("BLTouch ALARM or TRIGGER after STOW, recovering");
        #endif
      ...
      #if ENABLED(DEBUG_LEVELING_FEATURE)
        if (DEBUGGING(LEVELING)) SERIAL_ECHO("bltouch.stow_proc() end");
      #endif
    ...
    bool bltouch_status_proc() {
      ...
      #if ENABLED(DEBUG_LEVELING_FEATURE)
        if (DEBUGGING(LEVELING)) SERIAL_ECHO("BLTouch STATUS requested");
      #endif
        ...
        constexpr bool should_set = true;
          ...
          if (DEBUGGING(LEVELING)) {
            SERIAL_ECHOLNPAIR("last_written_mode - ", (int)bltouch_last_written_mode);
              ...
              SERIAL_ECHOLNPGM("config mode - BLTOUCH_SET_5V_MODE");
            #else
              SERIAL_ECHOLNPGM("config mode - OD");
            #endif
          }
        #endif
