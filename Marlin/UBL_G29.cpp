#include "Marlin.h"
  ...
  extern void prt_hex_word(unsigned int w); 
  extern float code_value_float(); 
  extern bool code_value_bool();
  extern bool code_has_value();
  extern float probe_pt(float x, float y, bool, int); 
  extern float zprobe_zoffset;
  extern void prt_hex_byte(unsigned int);
  extern bool set_probe_deployed(bool); //bob
  #define DEPLOY_PROBE() set_probe_deployed(true)  //bob
  #define STOW_PROBE() set_probe_deployed(false)  //bob
  bool ProbeStay = true;  //bob
  ...
  #define SIZE_OF_LITTLE_RAISE 0  //bob
  #define BIG_RAISE_NOT_NEEDED 0  //bob
  ...
  */
  ...
  static int G29_Verbose_Level = 0, UBL_MESH_Map_Flag = 0, Test_Value = 0;
...
SERIAL_PROTOCOLLN(Unified_Bed_Leveling_EEPROM_start);
        ...
        case 0: 
          for (i = 0; i < UBL_MESH_NUM_X_POINTS; i++ ) {         // Create a bowl shape.  This is
            for (j = 0; j < UBL_MESH_NUM_Y_POINTS; j++ ) {       // similar to what a user would see with
              Z1 = (((float) UBL_MESH_NUM_X_POINTS) / 2.0) - i;  // a poorly calibrated Delta.
              Z2 = (((float) UBL_MESH_NUM_Y_POINTS) / 2.0) - j;
    ...
    if ( code_seen('P') ) {
      ...
      switch (Phase_Value) {
        ...
        case 2:
          ...
          Height_Value = Z_CLEARANCE_BETWEEN_PROBES;
          if ( code_seen('H') ) {
            if ( code_has_value() ) {
  ...
  struct vector tilt_mesh_based_on_3pts(float pt1, float pt2, float pt3) {
    ...
    d = normal.dx*ubl_3_point_1_X + normal.dy*ubl_3_point_1_Y + normal.dz*pt1;
    c = -((normal.dx*ubl_3_point_1_X + normal.dy*ubl_3_point_1_Y) - d);
    SERIAL_ECHOPGM("d from first  point: ");
    SERIAL_PROTOCOL_F(d,6);
    SERIAL_ECHOPGM("  c: ");
    SERIAL_PROTOCOL_F(c,6);
    SERIAL_ECHOPGM("\n");
    d = normal.dx*ubl_3_point_2_X + normal.dy*ubl_3_point_2_Y + normal.dz*pt2;
    c = -((normal.dx*ubl_3_point_2_X + normal.dy*ubl_3_point_2_Y) - d);
    SERIAL_ECHOPGM("d from second point: ");
    SERIAL_PROTOCOL_F(d,6);
    SERIAL_ECHOPGM("  c: ");
    SERIAL_PROTOCOL_F(c,6);
    SERIAL_ECHOPGM("\n");
    d = normal.dx*ubl_3_point_3_X + normal.dy*ubl_3_point_3_Y + normal.dz*pt3;
    c = -((normal.dx*ubl_3_point_3_X + normal.dy*ubl_3_point_3_Y) - d);
  ...
  void G29_What_Command() {
    ...
    SERIAL_ECHOPAIR("  state:", digitalRead(KILL_PIN ) );
    SERIAL_PROTOCOLPGM("  \n");
