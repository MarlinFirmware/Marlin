  ...
  // Slow down when the buffer starts to empty, rather than wait at the corner for a buffer refill
    ...
    if (moves_queued > 1 && moves_queued < (BLOCK_BUFFER_SIZE) / 2) {
      if (segment_time < min_segment_time) {
        ...
        #ifdef XY_FREQUENCY_LIMIT || ENABLED(ENSURE_SMOOTH_MOVES)
      ...
      unsigned long segment_time = lround(1000000.0 / inverse_mm_s);
    #endif
    if (segment_time < (MIN_BLOCK_TIME * 1000)) {
      // buffer will be draining, set to MIN_BLOCK_TIME.
      inverse_mm_s = 1000000.0 / (MIN_BLOCK_TIME * 1000);
      segment_time = lround(MIN_BLOCK_TIME * 1000);
    }
