...
float bsteps = 0;	//Based on Ziggy's Z-Step display instruction	//ROBO-BH

  static void _lcd_babystep(int axis, const char *msg) {
    if (encoderPosition != 0) {
      babystepsTodo[axis] += (int)encoderPosition;
      bsteps = bsteps + (int)encoderPosition;	//Based on Ziggy's Z-Step display instruction	//ROBO-BH
      encoderPosition = 0;
      lcdDrawUpdate = 1;
    }
    
    // if (lcdDrawUpdate) lcd_implementation_drawedit(msg, "");
    if (lcdDrawUpdate) lcd_implementation_drawedit(msg, ftostr4(bsteps));	//Based on Ziggy's Z-Step display instruction	//ROBO-BH
...
// Convert float to string with +1234 format	//ROBO-BH
//Based on Ziggy's Babysteps display instruction	//ROBO-BH
char *ftostr4(const float &x) {			//ROBO-BH
  long xx = x * 10;						//ROBO-BH
  conv[0] = (xx >= 0) ? '+' : '-';		//ROBO-BH
  xx = abs(x);							//ROBO-BH
  conv[1] = (xx / 10000) % 10 + '0';	//ROBO-BH
  conv[2] = (xx / 1000) % 10 + '0';		//ROBO-BH
  conv[3] = (xx / 100) % 10 + '0';		//ROBO-BH
  conv[4] = (xx / 10) % 10 + '0';		//ROBO-BH
  conv[5] = 0;							//ROBO-BH
  return conv;							//ROBO-BH
}	//End of section based on Ziggy's Babysteps display instruction		//ROBO-BH
