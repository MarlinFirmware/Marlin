...
// commands
//#define GRADIENT_MIX           // Support for gradient mixing with M166 and
// LCD
//#define MIXING_PRESETS         // Assign 8 default V-tool presets for 2 or 3
// MIXING_STEPPERS
...
// nozzle
//#define HOTEND_OFFSET_Y { 0.0, 5.00 }  // (mm) relative Y-offset for each
// nozzle
//#define HOTEND_OFFSET_Z { 0.0, 0.00 }  // (mm) relative Z-offset for each
// nozzle
...
// with M80
//#define PSU_POWERUP_DELAY      250    // (ms) Delay for the PSU to warm up to
// full power
//#define LED_POWEROFF_TIMEOUT 10000    // (ms) Turn off LEDs after power-off,
// with this amount of delay

//#define POWER_OFF_TIMER               // Enable M81 D<seconds> to power off
// after a delay
//#define POWER_OFF_WAIT_FOR_COOLDOWN   // Enable M81 S to power off only after
// cooldown

//#define PSU_POWERUP_GCODE  "M355 S1"  // G-code to run after power-on (e.g.,
// case light on)
//#define PSU_POWEROFF_GCODE "M355 S0"  // G-code to run before power-off (e.g.,
// case light off)
...
// this temperature
//#define AUTO_POWER_CHAMBER_TEMP  30 // (°C) PSU on if the chamber is over this
// temperature
//#define AUTO_POWER_COOLER_TEMP   26 // (°C) PSU on if the cooler is over this
// temperature
#endif
#endif
 ...
 * using an SPI Thermocouple. By default,
 *        Hardware SPI on the default serial bus is used. If you have also set
 * TEMP_n_SCK_PIN and TEMP_n_MISO_PIN,
 *        Software SPI will be used on those ports instead. You can force
 * Hardware SPI on the default bus in the
 *        Configuration_adv.h file. At this time, separate Hardware SPI buses
 * for sensors are not supported.
 *
 ...
 *     5 : 100kΩ  ATC Semitec 104GT-2/104NT-4-R025H42G - Used in ParCan, J-Head,
 * and E3D, SliceEngineering 300°C
 ...
 *    11 : 100kΩ  Keenovo AC silicone mats, most Wanhao i3 machines - beta 3950,
 * 1%
 *    12 : 100kΩ  Vishay 0603 SMD NTCS0603E3104FXT (#8) - calibrated for Makibox
 * hot bed
 *    13 : 100kΩ  Hisens up to 300°C - for "Simple ONE" & "All In ONE" hotend -
 * beta 3950, 1%
 *    15 : 100kΩ  Calibrated for JGAurora A5 hotend
 *    18 : 200kΩ  ATC Semitec 204GT-2 Dagoma.Fr - MKS_Base_DKU001327
 *    22 : 100kΩ  GTM32 Pro vB - hotend - 4.7kΩ pullup to 3.3V and 220Ω to
 * analog input
 *    23 : 100kΩ  GTM32 Pro vB - bed - 4.7kΩ pullup to 3.3v and 220Ω to analog
 * input
 *    30 : 100kΩ  Kis3d Silicone heating mat 200W/300W with 6mm precision cast
 * plate (EN AW 5083) NTC100K - beta 3950
 ...
 * Thermistor
 *
 *  Analog Thermistors - 1kΩ pullup - Atypical, and requires changing out the
 * 4.7kΩ pullup for 1kΩ.
 ...
 *                NOTE: (1) Must use an ADC input with no pullup. (2) Some
 ...
 * voltage (STM32, LPC176x....) and 5V INA826 amplifier board supply.
 *                NOTE: ADC pins are not 5V tolerant. Not recommended because
 * it's possible to damage the CPU by going over 500°C.
 ...
 * temperature sensor, and another
 * sensor you'd like it to be redundant for. If the two thermistors differ by
 * TEMP_SENSOR_REDUNDANT_MAX_DIFF (°C),
 * the print will be aborted. Whichever sensor is selected will have its normal
 * functions disabled; i.e. selecting
 ...
 * by many degrees
 * (especially before PID tuning). Setting the target temperature too close to
 * MAXTEMP guarantees
 * a MAXTEMP shutdown! Use these values to forbid temperatures being set too
 * close to MAXTEMP.
 */
...
//'M303 D' to toggle activation.
//#define PID_PARAMS_PER_HOTEND // Use separate PID parameters for each extruder
 ...
 * correctly
 * this gives better responsiveness and stability than PID and it also removes
 * the need
 * for PID_EXTRUSION_SCALING and PID_FAN_SCALING. Use M306 T to autotune the
 * model.
 * @section mpctemp
 */
#if ENABLED(MPCTEMP)
//#define MPC_EDIT_MENU                             // Add MPC editing to the
//"Advanced Settings" menu. (~1300 bytes of flash)
//#define MPC_AUTOTUNE_MENU                         // Add MPC auto-tuning to
// the "Advanced Settings" menu. (~350 bytes of flash)
 ...
 * significantly
 * impact FET heating. This also works fine on a Fotek SSR-10DA Solid State
 * Relay into a 250W
 * heater. If your configuration is significantly different than this and you
 * don't understand
 * the issues involved, don't use bed PID until someone else verifies that your
 * hardware works.
 * @section bed temp
 */
#define PIDTEMPBED
 ...
 * hysteresis).
 * When set to any value below 255, enables a form of PWM to the bed that acts
 * like a divider
 * so don't use it unless you are OK with PWM on your bed. (See the comment on
 * enabling PIDTEMPBED)
 */
 ...
 * heater. If your configuration is significantly different than this and you
 * don't understand
 * the issues involved, don't use chamber PID until someone else verifies that
 * your hardware works.
 ...
 * hysteresis).
 * When set to any value below 255, enables a form of PWM to the chamber heater
 * that acts like a divider
 * so don't use it unless you are OK with PWM on your heater. (See the comment
 * on enabling PIDTEMPCHAMBER)
 */
...
// control only the heating element
 ...
 * others.
 * This brief delay results in more material deposition due to the pressure in
 * the nozzle.
 *
 * Current Kinematics and feedrate scaling deals with this by making the
 * movement as fast
 * as possible. It works for slow movements but doesn't work well with fast
 * ones. A more
 ...
 * rotating at the
 * center point, more likely it will burn more material than it should.
 * Therefore similar
...
// of the two wire
// ends are controlled by parallel axes (Y, J). Joints are directly mapped to
// axes (no kinematics).
...
// probe.
// Almost all printers will be using one per axis. Probes will use one or more
// of the
 ...
 * edge of the bed.
 * By default, the magnet is assumed to be on the left and activated by a home.
 * If the magnet is
 ...
 * https://reprap.org/wiki/Kossel#Automatic_bed_leveling_probe
...
// touch probe on G29,
// if servo actuated touch probe is not defined. Uncomment as appropriate for
// your printer/probe.
...
//(to improve accuracy)
//#define WAIT_FOR_HOTEND         // Wait for hotend to heat back up between
// probes (to improve accuracy & prevent cold extrude)
#endif
...
// commands (e.g., "M600 T%c")
// NOTE: After 'M412 H1' the host handles filament runout and this script does
// not apply.
...
// default
//#define FYSETC_MINI_12864_1_2    // Type C/D/E/F. Simple RGB Backlight (always
// on)
 ...
 * IA_CREALITY (T5UID1)
 *  - Download
 * https://github.com/InsanityAutomation/Marlin/raw/CrealityDwin2.0_Bleeding/TM3D_Combined480272_Landscape_V7.7z
 *  - Copy the downloaded DWIN_SET folder to the SD card.
 *
...
//#define ANYCUBIC_LCD_GCODE_EXT  // Add ".gcode" to menu entries for DGUS clone
// compatibility
#endif
 ...
 *
 * NOTOSANS  - Default font with antialiasing. Supports Latin Extended and
 * non-Latin characters.
 * UNIFONT   - Lightweight font, no antialiasing. Supports Latin Extended and
 * non-Latin characters.
 * HELVETICA - Lightweight font, no antialiasing. Supports Basic Latin
 * (0x0020-0x007F) and Latin-1 Supplement (0x0080-0x00FF) characters only.
 */
...
// frequency
// which is not as annoying as with the hardware PWM. On the other hand, if this
// frequency
